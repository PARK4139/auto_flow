...
def ensure_value_completed_2025_10_10(key_name, func_n, options=None, guide_text=None, history_reset=False):
    import logging
    import os
    ...
    from pk_internal_tools.pk_functions.get_hashed_items import get_hashed_items  # Changed
    from pk_internal_tools.pk_functions.get_str_removed_bracket_hashed_prefix import get_str_removed_bracket_hashed_prefix  # Changed
    ...
    # prompt_label = get_pk_label_by_via(file_id)
    # prompt_label_guide_text = get_user_input_guide_text(prompt_label)
    ...
    hashed_options = get_hashed_items(options)  # Changed
    numbered_selected = get_value_from_fzf_routine(
        file_id=file_id,
        options=hashed_options,  # Changed
        editable=editable,
        query=last_selected,
        guide_text=guide_text,
        fzf_theme=PkFzfTheme(layout_reverse=True),
    )

    selected = get_str_removed_bracket_hashed_prefix(numbered_selected)  # Changed
