...
### **테스트 워크플로우 규칙**
- **수동 테스트**: `ensure_pk_scenario_executed()` 함수를 통해 주요 기능의 시나리오를 수동으로 테스트합니다.
...
- **virtual environment 규칙**: `.venv` 디렉토리는 사용하지 않음, `.venv`와 `.venv`만 사용
- **OS별 virtual environment **: Windows에서는 `.venv`, Linux/WSL에서는 `.venv` 사용
- **서비스 환경**: Linux/Docker + docker-compose 기반 환경 구성
- **CLI 래퍼**: `pk_ensure` 접두사로 시작하는 파일들은 CLI 모드 실행 시 래퍼 파일

### **자동 실행 규칙**
- **가상 환경 직접 활성화**: `uv run`을 사용하는 대신, OS에 맞는 가상 환경을 직접 활성화하여 스크립트를 실행합니다. (예: Windows에서는 `.venv\Scripts\activate` 실행 후 `python a.py`)
...
- **GEMINI 전체작업 완료 알림**: GEMINI 전체작업이 완료될 때마다 (반복적으로) `pk_ensure_gemini_cli_worked_done.py`를 실행하여 음성으로 알림. 
  - **실행 방법**: `.venv\Scripts\activate`를 실행하여 가상 환경을 활성화한 후, `python C:\Users\your_username\Downloads\pk_system\pk_internal_tools\pk_wrappers\pk_ensure_gemini_cli_worked_done.py` 명령을 실행합니다.
- **GEMINI 전체작업 완료 알림 예외**: `pk_ensure_gemini_cli_worked_done.py` 스크립트는 사용자의 사전 동의를 얻었으므로, 작업 완료 후 별도의 확인 없이 즉시 실행합니다.
- **GEMINI 작업 완료 알림 실행 시점**: `ensure_gemini_cli_worked_done.py`는 Gemini가 사용자로부터 받은 **모든 작업 요청을 완전히 완료했을 때 한 번만 수행**하며, 이후 다음 작업 요청을 기다립니다.
- **Cursor 작업 완료 알림**: 작업이 끝나거나 테스트가 필요하여 사용자에게 알리기 직전에 `pk_ensure_cursor_worked_done.py` 스크립트를 실행하여 알림. 
  - **실행 방법**: `.venv\Scripts\activate`를 실행하여 가상 환경을 활성화한 후, `python C:\Users\your_username\Downloads\pk_system\pk_system\pk_internal_tools\pk_wrappers\pk_ensure_cursor_worked_done.py` 명령을 실행합니다.
  - **실행 시점**: 작업 완료 또는 테스트 필요 시 사용자에게 알리기 직전에 실행합니다.
- **Cursor 작업 완료 알림 예외**: `pk_ensure_cursor_worked_done.py` 스크립트는 사용자의 사전 동의를 얻었으므로, 작업 완료 후 별도의 확인 없이 즉시 실행합니다.
...
### **프로젝트 구조 이해**
- **최상위 Python 패키지**: `pk_internal_tools/`가 **메인 Python 패키지**이며, 프로젝트 루트에 직접 위치합니다.
- **메인 함수**: `pk_internal_tools/pk_functions/`이 **핵심 함수들**이 위치한 디렉토리입니다.
- **실행 스크립트**: `pk_internal_tools/pk_wrappers/pk_ensure_*.py` 파일들이 **직접 실행 가능한 래퍼 스크립트**들입니다.
...
- **파일명 규칙**: `.py`, `.sh`, `.cmd`, `.bat`, `.ps1` 파일에 `ensure_` 접두사. (단, `is_` 또는 `get_`으로 시작하는 파일은 예외)
- **변수 명명 규칙**: 변수명은 snake_case를 사용하며, 문자열 타입 변수에는 `_str` 접미사를 붙이지 않습니다.
- **테스트 코드**: `test_` 접두사로 테스트 파일 작성
- **불리언 반환 함수 명명 규칙**: `bool` 타입을 반환하는 함수는 `is_` 접두사로 시작해야 합니다. `check_` 접두사는 사용하지 않습니다.
  - **예시**:
    - ✅ `def is_vscode_installed() -> bool:`  # 올바른 명명
    - ✅ `def is_wifi_connected() -> bool:`  # 올바른 명명
    - ❌ `def check_vscode_installed() -> bool:`  # 규칙 위반
    - ❌ `def check_wifi_connection() -> bool:`  # 규칙 위반
  - **기존 함수 리팩토링**: 기존 `check_`로 시작하는 `bool` 반환 함수는 `is_`로 시작하도록 리팩토링해야 합니다.
- **로깅 포맷 규칙**: 로깅 시 f-string 사용을 선호합니다. `%` 포맷팅 대신 f-string을 사용하세요.
  - **예시**:
    - ✅ `logger.info(f"값: {value}")`  # f-string 사용
    - ✅ `logger.debug(f"exit_code: {exit_code}, stdout: {stdout}")`  # 여러 변수 포함
    - ❌ `logger.info("값: %s", value)`  # % 포맷팅 사용 (권장하지 않음)
    - ❌ `logger.debug("exit_code: %s", exit_code)`  # % 포맷팅 사용 (권장하지 않음)
- **입력 질의 규칙**: 연속 실행 프로그램에서 사용자에게 질의할 때의 input 규칙 (규칙 정의 위치: `GEMINI.md`, `.cursor/rules`의 "코드 작성 규칙" 섹션)
  - **y/n 답변이 필요한 경우**: `input("continue:y/n")` 사용
  - **단순히 진행에 필요한 경우**: `ensure_console_paused()` 사용
  - **예시**:
    - ✅ `answer = input("continue:y/n").strip().lower()`  # 사용자가 y/n 선택 필요
    - ✅ `ensure_console_paused()`  # Enter만 누르면 진행
    - ❌ `input("continue:")`  # 형식 불명확
    - ❌ `input("press enter to continue...")`  # 규칙 미준수
- **텍스트 작성 규칙**: 모든 텍스트(사용자 안내 메시지, 로그 메시지, TTS 메시지 등)는 간결하고 명시적으로 작성합니다.
  - **간결성**: 불필요한 단어나 장황한 설명 제거
  - **명시성**: 사용자가 수행해야 할 행동이나 확인해야 할 정보를 명확하게 표현
  - **우선순위**: 여러 조건이 있을 경우, 가장 명확한 조건만 선택하여 표현
  - **함수명/모듈명 표기 금지**: 텍스트 메시지에 함수명이나 모듈명을 대괄호로 감싸서 표기하지 않습니다. 의미 있는 메시지를 직접 작성합니다.
    - **예시**:
      - ✅ `logging.debug(f"대기 시간 측정 시작: {instruction_msg}")`  # 의미 있는 메시지
      - ✅ `logging.debug(f"PkQdialog 생성 성공")`  # 명확한 메시지
      - ❌ `logging.debug(f"[_measure_and_record_duration] 시작: ...")`  # 불필요한 함수명 표기
      - ❌ `logging.debug(f"[should_i_do] PkQdialog 생성 성공")`  # 불필요한 함수명 표기
      - ❌ `logging.debug(f"[pk_gui.py:1655] dialog 생성 시작")`  # 불필요한 파일명/줄번호 표기
  - **일반 예시**:
    - ✅ "마우스 왼쪽 버튼을 클릭하세요"
    - ✅ "키보드 Enter 키를 누르세요"
    - ✅ "대기 시간 측정: 마우스를 클릭하세요"
    - ❌ "대기 시간 측정을 시작합니다. 마우스를 클릭하거나 키보드 키를 누르거나 이벤트를 발생시키세요. 측정이 종료됩니다."
    - ❌ "뭔가를 하세요"
...
- **경로 결합**: `Path(userprofile) / "Downloads" / "pk_system"` 형태로 경로 결합
- **경로 검증**: `Path.exists()`, `Path.is_dir()`, `Path.is_file()` 등으로 경로 유효성 검증

### **테스트 코드 규칙**
- **테스트 파일**: `test_` 접두사를 붙여 명시적으로 작성
- **테스트 위치**: `pk_tests` 폴더에서 작성
- **자동화된 테스트**: 테스트 코드 생성 시, 테스트 대상 함수는 개발자 상호작용(interactive) 모드가 아닌 자동화된(non-interactive) 모드로 동작하도록 작성해야 합니다.
- **테스트 코드 초기화**: 모든 테스트 파일 상단에 `ensure_pk_wrapper_starting_routine_done(traced_file=__file__, traceback=traceback)`를 호출하여 프로젝트의 표준 초기화를 수행합니다. 
  - **예시**:
    ```python
    import traceback
    ...
    ```
- **로깅 설정**: 테스트 코드 작성 시, `logging.basicConfig()`를 호출하지 않고 `ensure_pk_system_log_initialized(__file__)`를 호출하여 프로젝트의 표준 로깅을 사용합니다. (`ensure_pk_wrapper_starting_routine_done` 내부에서 자동 호출됨)
- **출력 규칙**: 테스트 코드에서 `print()` 문이나 `ensure_printed()` 함수를 사용하지 않고, 모든 출력은 `logging.debug()`를 사용합니다. 
  - **예시**:
    - ✅ `logging.debug(f"테스트 결과: {result}")`
    - ❌ `print(f"테스트 결과: {result}")`
    - ❌ `ensure_printed(f"테스트 결과: {result}")`
- **특정 테스트 래퍼**: `pk_ensure_pk_scenario_executed.py` 파일은 테스트 시나리오 실행을 위한 특정 래퍼 스크립트입니다.

### **테스트 실행 환경 규칙**
- **virtual environment 자동 활성화**: 테스트 실행 시 OS 환경에 따라 자동으로 적절한 virtual environment 활성화
- **Windows 환경**: `.venv` 디렉토리의 virtual environment 사용 (`uv` 또는 `python -m venv`)
- **Linux/WSL 환경**: `.venv` 디렉토리의 virtual environment 사용 (`uv` 우선, 없으면 `python3`)
...
- **자동 테스트 환경**: 자동 테스트는 `uv .venv` 환경에서 수행해야 합니다.

### **LTA (Local Test Activate) 규칙**
- **테스트 모드 플래그**: `LTA` 변수를 통해 테스트 모드와 일반 모드 구분
- **테스트 환경 감지**: `pk_internal_tools/pk_objects/pk_lta.py`에서 LTA 상태 확인
...
- **OS별 virtual environment **: `.venv` (Windows), `.venv` (Linux/WSL) 사용
...
- **function_split 대체**: 기존 `pk_external_tools/function_split/` 대신 `pk_internal_tools/pk_functions/` 사용
...
- **파일명 규칙**: 객체 파일명은 기능별로 명확하게 구분 (예: `jarvis_state.py`, `losslesscut_pk_event.py`)
...
- **창작 방향 설정**: 프로젝트의 방향성이나 구조에 대한 튜토리얼
- **개념 설명 요청**: 특정 기술이나 개념에 대한 이해 도움
- **자료 정리 및 요약**: 복잡한 정보의 정리 및 핵심 요약
- **연구 방향 설정**: 기술 조사나 연구 주제 설정에 대한 튜토리얼
...
### **프로젝트 구조 규칙**
- **트리구조**:
  - `pk_internal_tools/`: **메인 Python 패키지 디렉토리.** 파이썬 핵심 소스 코드와 모듈을 포함합니다.
  - `pk_external_tools/`: 정적 파일 및 외부 소스 리소스 디렉토리.
  - `pk_tests/`: 테스트용 스크립트, 파일명에 `test_` prefix 패턴적용.
  - `pk_prompts/`: AI 기반 코드생성 명령용 프롬프트 모음. (현재 구조에는 없지만 규칙에 따라 추가될 예정입니다.)
  - `pk_logs/`: 프로그램 실행 결과를 저장하는 로그 디렉토리.
  - `pk_docs/`: 프로젝트 문서 디렉토리.
  - `pk_cache/`: 시스템 캐시 파일.
  - `.pk_system/`: 민감한 정보 및 사용자별 설정 파일.
...
- **page 서버, api 서버 url 라우터에 적용**:
  ...
  - `heal_base_hospital_worker/v1/page/ensure/login-guide/`: 로그인 (튜토리얼)
  ...
  - `heal_base_hospital_worker/v1/page/ensure/logined/and/hospital-location-guided/{실}`: 실별위치튜토리얼 + 광고
...
### **Event 시스템 구조**
- **핵심 클래스**: `pk_internal_tools/pk_objects/pk_event.py`에 기본 Event 시스템 클래스들 배치
...
- **로깅 위치**: `pk_internal_tools/pk_objects/pk_files.py`에 정의된 `F_PK_LOG` 경로 사용
...
### **PEP 8 기본 준수 규칙**
- **PEP 8 기본 준수**: 이 프로젝트는 **PEP 8 (Python Enhancement Proposal 8)**을 기본적으로 따릅니다. 모든 Python 코드는 PEP 8 스타일 가이드를 기본으로 작성합니다.
- **우선순위**: 프로젝트 고유 규칙을 최우선으로 하며, 그 다음으로 PEP 8(스타일 가이드) 및 PEP 20(The Zen of Python)을 포함한 최신 PEP 가이드라인을 준수합니다.
- **코드 품질**: 모든 Python 코드는 가독성, 유지보수성, 일관성을 최우선으로 작성합니다.
- **주요 PEP 8 규칙**:
  - 변수명 및 함수명: `snake_case` 사용
  - 클래스명: `PascalCase` 사용
  - 상수: `UPPER_CASE` 사용 (단, 프로젝트 내부 작업 목록 등은 `snake_case`로 통일)
  - 최대 줄 길이: 코드 79자, 주석 72자
  - 들여쓰기: 스페이스 4개
  - Import 순서: 표준 라이브러리 → 서드파티 → 로컬 모듈

### **Docstring 작성 규칙 (Google Style)**
- **Docstring 스타일**: 모든 Python 코드의 docstring은 Google 스타일을 따릅니다.
- **일관성 유지**: 프로젝트 전체에서 Google 스타일 docstring을 일관성 있게 사용합니다.
- **예시 (Google Style Docstring)**:
  ```python
  def ensure_example_function(param1: str, param2: int) -> bool:
      """이 함수는 예시를 보여줍니다.

      Args:
          param1 (str): 첫 번째 매개변수에 대한 설명입니다.
          param2 (int): 두 번째 매개변수에 대한 설명입니다.
...
def ensure_example_function():
    ...
    try:
        from pk_internal_tools.pk_objects.pk_directories import D_PROJECT, D_PK_TESTS
    except ImportError:
        # fallback: 직접 경로 계산
        from pathlib import Path
        D_PROJECT = str(Path(__file__).resolve().parent.parent.parent)
        D_PK_TESTS = str(Path(D_PROJECT) / "tests")
    
    # 함수 로직 시작
    test_target = Path(D_PK_TESTS) / "test_file.py"
    # ... 나머지 로직
```
...
### **Import Fallback 규칙**
- `ImportError` 발생 시 `try...except ImportError` 블록 내에서 폴백(fallback) 함수를 정의하는 패턴은 사용하지 않습니다. 대신, 필요한 모든 모듈은 직접 임포트하고, 임포트 실패는 상위 계층에서 처리하거나, 애플리케이션 시작 시점에 필수 종속성 검사를 통해 처리합니다.
...
### **func_n 인자 규칙**
- `ensure_value_completed`와 같은 사용자 입력 함수 호출 시 `func_n` 인자는 `get_caller_name()` 함수를 통해 동적으로 가져와야 합니다. 이는 파일명과 동일한 `key_name`을 유지하여 히스토리 관리의 일관성을 보장하기 위함입니다.

### **ensure_pk_wrapper_starting_routine_done 함수 인자 규칙**
- `ensure_pk_wrapper_starting_routine_done` 함수 호출 시 `traced_file` 인자를 사용해야 합니다. (예: `ensure_pk_wrapper_starting_routine_done(traced_file=__file__, traceback=traceback)`)

### **예외 처리 규칙**
- **함수 내 예외 처리**: 함수 구현 시 `try...except` 블록에서는 `ensure_debug_logged_verbose(traceback=traceback, e=e)`를 호출하여 예외를 처리합니다. 특히 `pk_internal_tools/pk_functions/` 내의 시스템 함수에서 이 규칙을 반드시 준수해야 합니다.
- **래퍼 스크립트**: 래퍼 스크립트에서는 `ensure_pk_wrapper_exception_routine_done(traced_file=__file__, traceback=traceback, e=e)`를 사용하여 예외를 처리합니다。
- **로깅**: 모든 예외 처리 시 `logging.error`를 사용하여 오류를 로깅하고, 필요한 경우 `exc_info=True`를 포함하여 트레이스백을 기록합니다。

### **사용자 입력 처리 규칙**
- **표준 함수 사용**: 사용자로부터 대화형 입력을 받을 때는 `input()` 함수 대신 `ensure_value_completed` (또는 `ensure_value_completed_YYYY_MM_DD_HHMM` 패턴의 최신 함수)를 사용합니다.
- **목적**: 이 함수들은 `fzf` 통합, 히스토리 관리, 옵션 제공 등 고급 사용자 경험을 제공하며, 프로젝트의 표준화된 입력 방식을 따릅니다.
- **활용**: `key_name`, `func_n`, `guide_text`, `options` 인자를 활용하여 입력의 목적과 가이드를 명확히 제공합니다。

### **fzf 사용 규칙**
- **기본 원칙**: 프로젝트에서 fzf 기능을 사용할 때는 기본적으로 `ensure_value~` 계열 함수를 사용해야 합니다.
- **단일 선택**: `ensure_value_completed` 또는 최신 버전의 `ensure_value_completed_YYYY_MM_DD_HHMM` 패턴 함수 사용
- **다중 선택**: `ensure_values_completed_2025_10_23` 사용
- **직접 fzf 호출 금지**: `subprocess`로 직접 fzf를 호출하거나 `get_pk_fzf_options()`를 사용하는 대신, `ensure_value~` 계열 함수를 사용합니다.
- **예외**: `ensure_value~` 함수 내부 구현에서는 직접 fzf를 호출할 수 있습니다.
- **예시**:
  - ✅ `selected = ensure_value_completed(key_name="파일 선택", func_n=func_n, options=file_list, guide_text="파일을 선택하세요:")`
  - ✅ `selected_files = ensure_values_completed(key_name="파일 선택", options=file_list, multi_select=True)`
  - ❌ `subprocess.run(["fzf", ...])`  # 직접 fzf 호출 (권장하지 않음)
  - ❌ `get_pk_fzf_options()` + 직접 fzf 실행 (권장하지 않음)

### **key_name 작성 규칙**
- **기본 원칙**: `ensure_value~` 계열 함수의 `key_name` 파라미터는 파일 ID 생성에 사용되므로 Windows 파일 시스템에 유효한 문자만 포함해야 합니다.
- **특수 문자 제거**: Windows 파일 시스템에서 유효하지 않은 문자(`< > : " / \ | ? *`)는 `key_name`에 포함하지 않습니다.
- **파일명 포함 시 sanitize**: `key_name`에 파일명이 포함되는 경우, 파일명의 특수 문자를 제거하거나 언더스코어(`_`)로 대체합니다.
- **guide_text와 분리**: 화면 표시용 텍스트는 `guide_text` 파라미터에 원본 텍스트(색상 코드 포함 가능)를 사용하고, `key_name`은 파일 ID 생성용으로 sanitize된 텍스트를 사용합니다.
- **예시**:
  - ✅ `key_name = "파일 선택"`  # 특수 문자 없음
  - ✅ `key_name = f"'{sanitized_file_name}'에 대해 어떤 작업을 수행하시겠습니까?"`  # 파일명 sanitize 후 사용
  - ✅ `guide_text = f"'{original_file_name}'에 대해 어떤 작업을 수행하시겠습니까?"`  # 원본 파일명 사용
  - ❌ `key_name = f"'{file_name_with_special_chars}'에 대해 어떤 작업을 수행하시겠습니까?"`  # 특수 문자 포함 (파일 경로 오류 발생 가능)
...
- **스크립트 파일 임베딩**: `.cmd`, `.sh`, `.ps1`, `.bat` 등 외부 스크립트 파일의 내용은 별도 파일로 생성하는 대신, `textwrap.dedent()`를 사용하여 파이썬 코드 내부에 f-string으로 임베딩하고, `ensure_script_file_created` 및 `ensure_command_executed_to_os_like_human`과 같은 함수를 통해 실행합니다.
...
def process_images_in_directory(directory_path, allowed_extensions=None):
    ...
    if allowed_extensions is None:
        from pk_internal_tools.pk_objects.pk_file_extensions import IMAGE_EXTENSIONS
...
```python
# 고성능 + 재사용 가능한 함수 설계
def ensure_target_files_classified_by_ngram(
    d_working, 
    ...
    """ngram 기반 파일 분류 및 정리 (고성능 + 재사용 가능)"""
...
### **핵심 디렉토리 구조 (프로젝트 루트 하위)**
- **`pk_external_tools/`**: 외부 실행 파일 및 리소스 디렉토리. (예: `pk_external_tools`)
- **`pk_internal_tools/`**: 파이썬 핵심 소스 코드 디렉토리. (예: `pk_internal_tools`)
- **`pk_tests/`**: 테스트 코드 디렉토리, 파일명에 `test_` prefix 패턴 적용. (예: `pk_tests`)
- **`pk_prompts/`**: AI 기반 코드생성 명령용 프롬프트 모음. (현재 구조에는 없지만 규칙에 따라 추가될 예정입니다.)
- **`pk_logs/`**: 프로그램 실행 로그 디렉토리. (예: `pk_logs`)
- **`pk_docs/`**: 프로젝트 문서 디렉토리. (예: `pk_docs`)
- **`downloaded_files/`**: 다운로드된 파일 저장 디렉토리. (예: `downloaded_files`)
- **`pk_sounds/`**: 사운드 파일 저장 디렉토리. (예: `pk_sounds`)

### **최상위 `.pk_system` 및 `pk_system/.pk_system` 디렉토리 구조**
- **`.pk_system/`**: 민감한 정보 및 사용자별 설정 파일. 프로젝트 루트 레벨에 위치합니다. (예: `C:\Users\your_username\Downloads\pk_system\.pk_system`)

- **`pk_cache/`**: 시스템 캐시 파일 디렉토리. `.pk_system` 디렉토리 하위에 위치합니다. (예: `.pk_system/pk_cache`)

### **명명 규칙 원칙**
- **`pk_external_tools/`**: 역사적인 이유로 사용되는 핵심 소스 디렉토리입니다.
- **`pk_*`**: `pk_external_tools` 하위에서 시스템 레벨의 공유 기능을 담당하는 디렉토리는 `pk_` 접두사를 사용하여 명확히 구분합니다.
- **`pkg_*` 접두사 사용 지양**: `pk_external_tools` 외에 새로운 디렉토리를 생성할 때는 `pkg_` 접두사 사용을 지양하고, 기능에 맞는 명확한 이름을 부여합니다.

### **디렉토리명 패턴**
- **`pk_`**: 프로젝트 접두사 (pk_system)
- **`system_`**: 시스템 레벨 구분
- **`pk_objects/`**: 객체, 클래스, 상태 관리
- **`pk_wrappers/`**: 파이썬 래퍼 스크립트만 포함. 다른 유형의 스크립트(cmd, sh, ps1 등)는 `pk_external_tools` 하위 디렉토리로 이동해야 합니다.
- **`pk_functions/`**: 핵심 함수들
...
**모든 디렉토리는 `pk_` 접두사를 사용하여 일관성 있게 명명합니다! 📁✨**
### **README 파일 작성 규칙**
- **영어 사용**: 모든 `README.md` 파일은 영어로 작성해야 합니다.
- **정보 전달**: 프로젝트의 목적, 설정 방법, 사용법 등을 명확하고 간결하게 설명합니다.
- **업데이트 유지**: 코드 변경 사항에 맞춰 `README.md` 파일도 항상 최신 상태로 유지합니다.
```