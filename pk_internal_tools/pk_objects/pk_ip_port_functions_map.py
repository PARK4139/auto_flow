...
class pk_ip_port_functions_map:
    ...
    # 
    # 서버 정보:
    #   - 이름: remote_target (Jetson AGX Xavier)
    ...
    #               Host Machine에서 remote_target_ip로 접속 가능합니다.
    ...
    #
    remote_target: Dict[str, ServerFunctionConfig] = {
        ServerFunctionName.DASHBOARD.value: ServerFunctionConfig(
            ...
            function_description=(
                ...
                "- 중지: Xavier에서 'pkill -f ensure_pk_dashboard_server_on_remote_target.py'\n"
            ...
            additional_info={
                ...
                "start_command": "nohup python3 /tmp/ensure_pk_dashboard_server_on_remote_target.py",
                "stop_command": "pkill -f ensure_pk_dashboard_server_on_remote_target.py"
            }
        ),
        ...
        ServerFunctionName.MONITOR.value: ServerFunctionConfig(
            ...
            function_description=(
                ...
                "- 중지: Xavier에서 'pkill -f ensure_pk_p110m_monitor_on_remote_target.py'\n"
                "- DB: ~/pk_system/.pk_system/pk_system.sqlite\n"
            ...
            additional_info={
                ...
                "start_command": "nohup python3 /tmp/ensure_pk_p110m_monitor_on_remote_target.py",
                "stop_command": "pkill -f ensure_pk_p110m_monitor_on_remote_target.py",
        ...
        ServerFunctionName.ARDUINO_DEV.value: ServerFunctionConfig(
            ...
            function_description=(
                ...
                "- 접속: VSCode Remote SSH로 remote_target 연결 후 PlatformIO 확장 설치\n"
    ...
    def get_server_info(cls, server_name: str, function_name: str) -> Optional[ServerFunctionConfig]:
        ...
        Args:
            server_name: 서버 이름 (예: "remote_target", "pk_asus")
        ...
        if ip is None:
            ...
            if env_var:
                try:
                    ...
                    ip = ensure_env_var_completed(env_var, default_value=default_ip)
