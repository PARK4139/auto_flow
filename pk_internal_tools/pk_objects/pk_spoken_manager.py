...
from pk_internal_tools.pk_objects import pk_tts_cache_utils
...
class SpokenManager:
    ...
    def __init__(self, verbose: bool = True):
        ...
        self._cache_dir = D_TTS_CACHE

        self.player = PygamePlayer(verbose=self.verbose)
        # 사용자의 요청에 따라 GTTSBackend만 사용하도록 수정
        self.backends: List[TTSBackend] = [
            GTTSBackend(self._cache_dir, verbose=self.verbose),
            # ElevenLabsBackend(self._cache_dir, verbose=self.verbose) # ElevenLabs는 fallback으로 사용하지 않음
        ]
        ...
        for backend in self.backends:
            try:
                ...
                if audio_path:
                    ...
                    if self.player.play(audio_path, self.voice_config.volume):
                        synthesized_successfully = True
                        return  # Stop after first successful playback
            except Exception as e:
                if self.verbose: logging.error(f"Backend {backend.__class__.__name__} failed: {e}", exc_info=True)

        if not synthesized_successfully:
            if self.verbose: logging.error("All TTS backends failed. Falling back to console output.")
            fallback_message = f"[음성 출력 실패] 다음 내용을 확인하세요:\n\n{text}"
            # GUI alert for immediate visual feedback
            # alert_as_gui(fallback_message, title="TTS Fallback")
            # Blocking console message to ensure user acknowledges it
            ensure_paused(f"{fallback_message}\n\nEnter를 눌러 계속 진행하세요...")
