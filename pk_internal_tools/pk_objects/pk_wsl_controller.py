...
from pk_internal_tools.pk_objects.pk_modes import PkModesForEnsureSlept, PkModesForGetListFromClass, PkModesForSdkManager
...
class PkWslController(PkIdentifier):
    ...
    def ensure_target_distro_package_installed(self, distro_package_name):
        ...
        if distro_package_name == 'docker':
            ...
            if ensure_signature_found_in_lines(signature='installed', lines=std_outs):
                logging.debug(f"{distro_package_name} is installed in {self._remote_target.distro_name}")
    ...
    def ensure_file_transferred_to_remote_target(self, local_path: str, remote_path: str) -> bool:
        ...
        logging.info(
            ...
            f"  - Host: {self._remote_target.ip}:{self._remote_target.port}\n"
            f"  - User: {self._remote_target.user_name}\n"
            f"  - Key File: {self._remote_target.f_local_ssh_private_key}\n"
        ...
        # n. 공개 키 인증 시도
        try:
            if self._remote_target.f_local_ssh_private_key:
                logging.info("SSH 공개 키로 연결을 시도합니다...")
                key_private = paramiko.Ed25519Key(file_name=self._remote_target.f_local_ssh_private_key)
                ssh.connect(hostname=self._remote_target.ip, port=self._remote_target.port, username=self._remote_target.user_name, pkey=key_private, timeout=10)
                connected = True
        ...
        if not connected and self._remote_target.pw:
            try:
                logging.info("SSH 암호로 연결을 시도합니다...")
                ssh.connect(hostname=self._remote_target.ip, port=self._remote_target.port, username=self._remote_target.user_name, password=self._remote_target.pw, timeout=10)
                connected = True
        ...
        try:
            ...
            sftp.close()
            logging.info(f"파일 전송 성공: {local_path} -> {self._remote_target.ip}:{remote_path}")
            return True
        except Exception as e:
    ...
    def ensure_command_to_remote_target_with_pubkey(self, cmd):
        ...
        if not self._remote_target or not self._remote_target.ip:
        ...
        try:
            ...
            if self._remote_target.pw:
                stdin.write(self._remote_target.pw + '\n')
                stdin.flush()
    ...
    def ensure_sdkmanager_executed_on_wsl_distro(self, setup_op: "PkModesForSdkManager" = PkModesForSdkManager.GUI):
        ...
        sdk_cmd = None
        if setup_op == PkModesForSdkManager.GUI:
            sdk_cmd = f"start wsl -d {distro_name} -u {wsl_user_n} -- sdkmanager"
        elif setup_op == PkModesForSdkManager.CLI:
    ...
    def get_wsl_distro_name_installed_legacy(self):
        ...
        if len(wsl_distro_names_installed) == 1:
            ...
            from pk_internal_tools.pk_functions.get_caller_name import get_caller_name
            func_n = get_caller_name()
            selected = ensure_value_completed(key_name=key_name, func_n=func_n, options=wsl_distro_names_installed)
            wsl_distro_name = selected
    ...
    def get_wsl_distro_pw_legacy(self, distro_name):
        from pk_internal_tools.pk_functions.get_caller_name import get_caller_name
        func_n = get_caller_name()
        env_var_name = get_pk_env_var_id("PASSWORD", func_n)
    ...
    def get_wsl_distro_ip_legacy(self, distro_name):
        ...
        from pk_internal_tools.pk_functions.is_os_windows import is_os_windows
        from pk_internal_tools.pk_functions.ensure_command_executed import ensure_command_executed
        from pk_internal_tools.pk_functions.get_pk_env_var_id import get_pk_env_var_id  # New import
        from pk_internal_tools.pk_functions.get_caller_name import get_caller_name
        func_n = get_caller_name()
        ...
        if not wsl_ip:
            env_var_name = get_pk_env_var_id("IP", func_n)  # Use helper function
    ...
    def get_wsl_distro_user_name_legacy_via_whoami(self, distro_name):
        ...
        if not user_n:
            from pk_internal_tools.pk_functions.get_caller_name import get_caller_name
            func_n = get_caller_name()
            env_var_name = get_pk_env_var_id("USERNAME", func_n)  # Use helper function
    ...
    def ensure_wsl_distro_sudoers_fragment_written(self, distro: str, file_name: str, content: str):
        tmp_path = f"/tmp/.{file_name}.tmp"
        remote_path = f"/etc/sudoers.d/{file_name}"
        cmd = (
    ...
    def ensure_wsl_distro_assistance_executed(self):
        ...
        while True:
            operation_option = ensure_value_completed(key_name="작업옵션", options=work_type)

            distro_name = None
            if operation_option != OP_DISTROS_INFO:
                key_name = "distro_name"
                wsl_distros_not_supported_officially_anymore = get_list_from_class(WslDistrosNotSupportedOfficiallyAnymore, PkModesForGetListFromClass.ATTRIBUTE_VALUE)
            ...
            if operation_option == OP_ENTER:
                ...
                if distro_name in unsupported_distro_values:
                    ...
                    ensure_slept(seconds=15, setup_op=PkModesForEnsureSlept.SILENT)
