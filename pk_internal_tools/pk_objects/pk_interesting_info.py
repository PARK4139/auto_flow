...
class PkInterestingInfos:
    ...
    def __str__(self):
        ...
        if self.date:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}날짜:{PkColors.RESET} {self.date}"
            )
        if self.time:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}시간:{PkColors.RESET} {self.time}"
            )
        if self.day_of_week:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}요일:{PkColors.RESET} {self.day_of_week}"
            )
        if self.location:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}위치:{PkColors.RESET} {self.location}"
            )
        if self.weather_infos:
            ...
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}날씨 정보:{PkColors.RESET}\n{weather_details}"
            )
        ...
        if self.os_info:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}OS 정보:{PkColors.RESET} {self.os_info}"
            )
        if self.screen_info:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}화면 정보:{PkColors.RESET}\n"
                f"{self.screen_info}"
            )
        if self.connected_drives_info:
            ...
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}연결된 드라이브 정보(C 드라이브 제외):{PkColors.RESET}\n"
        ...
        if self.wifi_profile_name:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}WiFi 프로필명:{PkColors.RESET} {self.wifi_profile_name}"
            )
        if self.wifi_password:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}WiFi 비밀번호:{PkColors.RESET} {ensure_sensitive_info_masked(self.wifi_password)}"
            )
        ...
        if self.top_cpu_processes:
            ...
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}CPU 상위 {len(self.top_cpu_processes)}개 프로세스:{PkColors.RESET}\n"
        ...
        if self.top_memory_processes:
            ...
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}메모리 상위 {len(self.top_memory_processes)}개 프로세스:{PkColors.RESET}\n"
        ...
        if self.ai_ide_processes_info:
            ...
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}AI IDE 프로세스:{PkColors.RESET}\n"
        ...
        if self.tasklist_info:
            ...
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}작업 목록(Image_Name PID):{PkColors.RESET}\n"
        ...
        if self.project_info:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}프로젝트 정보:{PkColors.RESET}\n"
                f"{self.project_info}"
            )
        if self.python_imports_info:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}Python 임포트 정보:{PkColors.RESET}\n"
        ...
        if self.lta is not None:
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}QC_MODE (Local Test Activate):{PkColors.RESET} {'활성화' if self.lta else '비활성화'}"
            )
        if self.window_titles:
            formatted_titles_str = '\n'.join([f"  {t}" for t in self.window_titles])
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}창 제목:{PkColors.RESET}\n"
        ...
        if self.stock_info:
            ...
            parts.append(
                f"{PK_UNDERLINE}\n"
                f"{PkColors.BRIGHT_CYAN}주식 정보 (# 데이터근거 및 출처):{PkColors.RESET}\n"
    ...
    def to_dict(self) -> dict:
        """
        객체의 정보를 딕셔너리 형태로 반환합니다.
        PkInterestingInfoSession에서 사용될 title과 content를 포함합니다.
        """
        title = "PK Interesting Infos"
