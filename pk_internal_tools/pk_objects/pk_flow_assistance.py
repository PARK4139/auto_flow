...
from pk_internal_tools.pk_functions.ensure_target_controlled import ensure_target_file_system_scanned
...
class PkScheduler:
    ...
    def __init__(self, state_file: Optional[str] = None, speak_mode: bool = True):
        ...
        from pk_internal_tools.pk_objects.pk_modes import PkModesForEnsureSlept
        ...
        if not is_initialized or settings_changed:
            ...
            if self.speak_mode:
                ...
                from pk_internal_tools.pk_objects.pk_modes import PlayerSelectionMode, PkModesForEnsureValueCompleted
                from pk_internal_tools.pk_functions.ensure_value_completed import ensure_value_completed

                func_n = get_caller_name()

                selection_mode_options = [member.value.lower() for member in PlayerSelectionMode]
                selected_mode_str = ensure_value_completed(
                ...
                # ensure_python_file_executed_advanced(file_path=D_PK_WRAPPERS / 'pk_ensure_pk_system_log_editable.py')
                ...
                ensure_slept(seconds=3, setup_op=PkModesForEnsureSlept.SILENT)
    ...
    def speak_routine_start(self, routine_title: str):
        ...
        try:
            ensure_spoken(text_to_speak, read_finished_wait_mode=True)
        finally:
    ...
    def _setup_completion_logger(self):
        ...
        log_file_name = Path(D_PK_LOGS) / f"pk_schedule_executed_at_{start_time_str}.log"
