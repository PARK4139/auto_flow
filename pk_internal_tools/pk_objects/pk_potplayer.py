...
class PkPotplayer(PkMediaFileController):
    ...
    def __init__(self, d_working: Path = None) -> None:
        ...
        self.f_player: Path = F_POTPLAYER_EXE
        self.file_to_load: Path | None = None
        self.file_to_allowed: list = []
        self.allowed_extensions: list[str] = list(PK_FILE_EXTENSIONS['videos'] | PK_FILE_EXTENSIONS['audios'])
        func_n = get_caller_name()

        # is_file_name_filter_on = bool(ensure_value_completed(key_name="is_file_name_filter", func_n=func_n, options=["True", "False"]))
        # self.file_name_parts_to_ignore: List[str] = ensure_condition_choosen(
        #     is_condition=is_file_name_filter_on,
        #     positive_return=FILENAME_PARTS_TO_IGNORE,
        #     negative_return=['without file_name fileter'],
        # )
        self.file_name_parts_to_ignore = []

        # is_regex_patterns_on = bool(ensure_value_completed(key_name="is_regex_patterns_on", func_n=func_n, options=["True", "False"]))
        # self.file_name_regex_patterns_to_ignore: List[str] = ensure_condition_choosen(
        #     is_condition=is_regex_patterns_on,
        #     positive_return=REGEX_PATTERNS_TO_IGNORE,
        #     negative_return=['without regex pattern'],
        # )
        self.file_name_regex_patterns_to_ignore = []
    ...
    def ensure_target_file_loaded(self, file_to_play: Path):
        ...
        try:
            if file_to_play and file_to_play.exists():
                # ensure_command_executed_2025_08_04_async(cmds=[self.f_player, file_to_play])
                # ensure_command_executed_2025_08_04(cmd=rf'start "" {get_text_chain(self.f_player, file_to_play)}')
                ensure_command_executed_2025_08_04(cmd=rf'start "" {get_text_chain(self.f_player, file_to_play)}', mode='a')
                return True
    ...
    def ensure_state_machine_executed(self) -> None:
        ...
        while 1:
            ...
            if self.is_player_opened():  # Checks if PotPlayer window is open
