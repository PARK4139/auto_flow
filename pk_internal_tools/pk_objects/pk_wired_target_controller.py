...
class PkWiredTargetController(PkIdentifier):
    ...
    def set_target(self, config: dict = None):
        from pk_internal_tools.pk_functions.ensure_env_var_completed_2025_11_24 import ensure_env_var_completed_2025_11_24
        from pk_internal_tools.pk_functions.get_caller_name import get_caller_name
        from pk_internal_tools.pk_objects.pk_identifier import PkDevice
        from pk_internal_tools.pk_objects.pk_qc_mode import QC_MODE
        ...
        if identifier is None:
            ...
            if QC_MODE:
                ...
                pk_device = ensure_env_var_completed_2025_11_24(
                    key_name=key_name,
                    func_n=func_n,
                    options=pk_devices
                )
    ...
    def ensure_wired_target_flashed(self):
        try:
            ...
            try:
                ...
                ok = ensure_value_completed_2025_10_12_0000(key_name=f"WSL 여유 공간 확인에 실패했습니다. 계속 진행하시겠습니까?", options=[PkTexts.YES, PkTexts.NO])
                if ok == PkTexts.NO:
                    return False
            # === End of Disk Space Check ===

            connected = is_internet_connected_2025_10_21()
            if not connected:
            ...
            if self._target.identifier == PkDevice.jetson_agx_xavier:
                ...
                if not l4t_path:
                    ...
                    self.ensure_sdkmanager_executed_on_wsl(setup_op=SetupOpsForSdkManager.CLI)
                    l4t_path = self.ensure_l4t_path_located(distro_name, target_user_name)
                    if not l4t_path:
                ...
                sdk_mode = ensure_value_completed_2025_10_13_0000(
                ...
                if sdk_mode == sdk_mode_options[0]:  # Interactive Mode
                    ...
                    # n. Download only
                    logging.info("자동 모드: 1. 파일 다운로드를 시작합니다 (downloadonly)...")
                    ...
                    logging.info("자동 모드: 1. 파일 다운로드 완료.")
                    ...
                    if not sample_rootfs_tbz:
                        logging.error("자동 모드 오류: Sample RootFS 파일(Tegra_Linux_Sample-Root-Filesystem_R35.6.1_aarch64.tbz2)을 찾을 수 없습니다.")
                        return False

                    # n. Extract Sample RootFS
                    logging.info(f"자동 모드: 2. Sample RootFS를 {l4t_dir}/rootfs에 압축 해제합니다...")
                    ...
                    logging.info("자동 모드: 2. Sample RootFS 압축 해제 완료.")

                    # 3. Apply Binaries (with fallback)
                    logging.info("자동 모드: 3. 바이너리를 적용합니다 (apply_binaries.sh)...")
                    ...
                    logging.info("자동 모드: 3. 바이너리 적용 완료.")

                    # 4. Flash
                    logging.info("자동 모드: 4. 플래시를 시작합니다 (flash.sh)...")
                    flash_cmd = f"cd {self.get_sh_quote(l4t_dir)} && ./flash.sh jetson-agx-xavier-devkit mmcblk0p1"
                ...
                # if not ensure_pk_xavier_auto_rebootable:
                #     ssh command -> restart
            ...
            while True:
                if is_os_windows():
                    ...
                    if bus_id is None:
                        ...
                        ok = ensure_value_completed_2025_10_13_0000(key_name=question, func_n=func_n, options=[PkTexts.YES, PkTexts.NO, PkTexts.FAILED], guide_text=guide_text)
                        if ok == PkTexts.YES:
                            continue
                        elif ok == PkTexts.NO:
    ...
    def ensure_target_info_printed(self):
        logging.debug(f'self.target.to_json()={self._target.to_json()}')
        return
    ...
    def ensure_target_effective_info_printed(self):
        ...
        # target 객체 → dict
        target_dict = self._target.to_dict()
        ...
        while True:
            ...
            if ok == PkTexts.YES:
                ...
                if self.get_uspipd_version():
                    ...
                    ensure_spoken(get_easy_speakable_text("설치 확인에 실패했습니다. 트러블슈팅 튜토리얼를 확인하고 재시도해주세요."), wait=True)
                    continue
    ...
    def ensure_sdkmanager_executed_on_wsl(self, setup_op: "SetupOpsForSdkManager" = SetupOpsForSdkManager.GUI):
