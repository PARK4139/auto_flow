...
class PkMediaFileController(ABC):
    ...
    def _get_d_working_from_options(self):
        ...
        from pk_internal_tools.pk_objects.pk_directories import D_PK_WORKING_S, D_DOWNLOADED_FROM_TORRENT
        from pk_internal_tools.pk_objects.pk_directories import D_PK_WORKING, D_G_DRIVE_PK_WORKING
        key_name = "d_working"
        func_n = get_caller_name()
        options = [Path(p) for p in [
            D_G_DRIVE_PK_WORKING,
            D_PK_WORKING,
            D_DOWNLOADED_FROM_TORRENT,
            D_PK_WORKING_S,
            Path(rf"F:\pk_archived\pk_sound\4"),
            Path(rf'F:\pk_archived\pk_s\pk_working'),
        ]]
    ...
    def get_d_working_from_options(self) -> Path | str:
        ...
        if QC_MODE:
            # d_working = Path('F:\pk_archived\pk_s\pk_working')
