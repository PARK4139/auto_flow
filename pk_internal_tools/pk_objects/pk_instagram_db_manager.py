...
class PkInstagramDB:
    def __init__(self):
        d_instagram_downloads_cache = Path(d_pk_cache) / "instagram_downloads"
    ...
    def add_urls_to_db(self, urls: list[str], cookie_opts: dict):
        ...
        with yt_dlp.YoutubeDL(validation_opts) as ydl:
            for raw_url in urls:
                ...
                if result:
                    ...
                    cur.execute(
                        "INSERT INTO instagram_urls (raw_url, video_id, download_status, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
                        (raw_url, video_id, "PENDING", datetime.now().isoformat(), datetime.now().isoformat()))
        self.pk_db.pk_db_connection.commit()
