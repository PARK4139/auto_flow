...
def ensure_home_assistant_ready_on_target(
    ...
    *,
    ...
    1) docker 컨테이너 상태 확인
    2) 필요 시 docker 설치 (target_controller.ensure_wireless_target_distro_docker_installed 활용)
    ...
    def _run(cmd: str, *, desc: str, use_sudo: bool = True, timeout: int = 120):
        ...
        stdout, stderr, exit_status = target_controller.ensure_command_to_wireless_target(
            cmd=cmd,
    ...
    try:
        ...
        target_controller.ensure_wireless_target_distro_docker_installed()
    except Exception as docker_error:
        logging.error("Docker 설치/구성 중 오류 발생: %s", docker_error)
        return False

    # 3) 설정 디렉터리 생성
    target_user = getattr(target_controller.wireless_target, "user_name", "pk")
    _run(
