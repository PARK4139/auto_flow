...
def ensure_instagram_videos_downloaded():
    ...
    try:
        ...
        if downloaded_files:
            play_option = ensure_value_completed_2025_10_12_0000(key_name="다운로드한 동영상을 재생할까요?", options=[PkTexts.SKIP, PkTexts.play])
...
def _get_urls_from_txt_file(file_path: Path) -> list[str]:
    ...
    while True:
        if ensure_value_completed_2025_10_12_0000(key_name="URL 입력을 완료하셨습니까?", options=[PkTexts.YES, PkTexts.NO]) == PkTexts.YES: break
