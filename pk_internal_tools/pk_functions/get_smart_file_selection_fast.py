...
def get_smart_file_selection_fast(pk_files, last_selected):
    ...
    from pk_internal_tools.pk_functions.get_file_names_to_display import get_file_names_to_display
    ...
    """초고속 파일 선택 로직"""
    try:
        display_names = get_file_names_to_display(files=pk_files)
        ...
        selected_name = ensure_value_completed(key_name=key_name, func_n=func_n, options=options)

        # selected_name = _ensure_value_completed_return_core(
        #     message=f"파일 선택",
        #     options=display_names
        # )
