...
def ensure_vscode_live_share_server_started_on_xavier(
    ...
    try:
        ...
        # Xavier 연결 정보 가져오기
        if not xavier_ip:
            xavier_ip = ensure_env_var_completed_2025_11_24(
                "XAVIER_IP",
                func_n=func_n,
                guide_text="Xavier IP 주소를 입력하세요:",
            )
        if not xavier_user:
            xavier_user = ensure_env_var_completed_2025_11_24(
                "XAVIER_USER",
                func_n=func_n,
                default_value="pk",
            )
        if not xavier_pw:
            xavier_pw = ensure_env_var_completed_2025_11_24(
                "XAVIER_PW",
                func_n=func_n,
        ...
        if not xavier_ip:
            logger.error("Xavier IP 주소가 입력되지 않았습니다.")
            return False
        ...
        try:
            ...
            if exit_code == 0 and stdout and len(stdout) > 0:
                ...
                if result == "exists":
                    ...
                    try:
                        from pk_internal_tools.pk_functions.ensure_vscode_remote_ssh_connected_to_xavier import (
                            ensure_vscode_remote_ssh_connected_to_xavier,
                        )
                        
                        logger.info("  pk_asus에서 VSCode Remote SSH 연결 시도 중...")
                        if ensure_vscode_remote_ssh_connected_to_xavier(
                            xavier_host="xavier",
                            xavier_ip=xavier_ip,
                            ...
                            logger.info(f"    3. '{xavier_user}@{xavier_ip}' 또는 'xavier' 선택")
            ...
            if exit_code == 0:
                ...
                if stdout and len(stdout) > 0:
                    ...
                    if extension_path and extension_path.lower() != "not_found" and "not_found" not in extension_path.lower():
                        ...
                        logger.info("    4. 'Install in SSH: xavier' 클릭")
                else:
