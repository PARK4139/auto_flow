...
from pk_internal_tools.pk_objects.pk_texts import PK_UNDERLINE_SHORT, PK_UNDERLINE_HALF


def find_and_replace_target_file_texts(file_path: str, old_text: str, new_text: str, case_sensitive: bool = False):
    """
    ...
    try:
        ...
        if new_content != content:
            ...
            logging.debug(f"{PkColors.BRIGHT_GREEN}파일: {file_path}{PkColors.RESET}")
            logging.debug(f"{PkColors.YELLOW}기존 텍스트: {old_text}{PkColors.RESET}")
            logging.debug(f"{PkColors.CYAN}새 텍스트: {new_text}{PkColors.RESET}")
        else:
