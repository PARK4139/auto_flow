...
def copy_pnx_from_local_os_to_remote_os_via_rsync(d_pnx):
    ...
    try:
        ...
        logging.debug(f'''d_pnx_new={d_pnx_new}  ''')
        ...
        if not os.path.exists(d_pnx_new):
            ...
            # logging.basicConfig(file_name=DIRSYNC_LOG, level=logging.DEBUG, filemode='w', encoding=Encoding.UTF8.value)
            ...
            #         PkGui.pop_up_as_complete(title_="작업성공보고", ment=f"타겟의 동기화가 성공 되었습니다\n{pnx_todo_new}", auto_click_positive_btn_after_milliseconds=1)
    ...
    # 윈도우 d를 WSL 경로로 변환
    # try:
    #     server_time=get_pk_time_2025_10_20_1159('%Y_%m_%d_%H_%M_%S_%f')
