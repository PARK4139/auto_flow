...
#
#         # 특정시간에 OS 부팅
#         # should_i_do(ment="백업할 타겟들을 크기에 따라 분류를 해둘까요", function=classify_pnxs_between_smallest_pnxs_biggest_pnxs, auto_click_positive_btn_after_milliseconds=10)
...
#
#         # mkr_특정시간에 한번
#         # if is_same_time(time1=get_pk_time()('%Y_%m_%d_%H_%M_%S_%f'), time2=datetime(year=2024, month=10, day=31, hour=22, minute=27 , second=0)):
...
#                 pk_state_address=f"state_pk_schedule/{yyyy}_{get_pk_time('%m')}_{get_pk_time('%d')}_daily_greeting")
#             if daily_greeting:
#                 daily_greeting = 0
...
#         #     state_toml[f'{yyyy}_{get_pk_time()('%m')}_{get_pk_time()('%d')}_greeting'] = 0
#         #     with open(F_QC_MODE_TOML, "w") as f:
#         #         toml.dump(state_toml, f)
#
#         if is_newyear():
#             state_toml = toml.load(F_QC_MODE_TOML)["state_pk_schedule"]
#             state_yearly_greeting = state_toml[f'{yyyy}_{get_pk_time('%m')}_{get_pk_time('%d')}_newyear_greeting']
#             if not state_yearly_greeting:
#                 state_toml[f'{yyyy}_{get_pk_time('%m')}_{get_pk_time('%d')}_newyear_greeting'] = 0
#                 state_yearly_greeting = 0
#             if state_yearly_greeting == 0:
#                 state_toml[f'{yyyy}_{get_pk_time('%m')}_{get_pk_time('%d')}_newyear_greeting'] = 1
...
#
#         if is_christmas():
#             # TODO state_toml is replaced by state of DB
#             state_christmas_greeting =  # f'{yyyy}_{get_pk_time('%m')}_{get_pk_time('%d')}_christmas_greeting'
#             if not state_christmas_greeting:
#                 state_christmas_greeting = 0
#                 state_toml[
#                     f'{yyyy}_{get_pk_time('%m')}_{get_pk_time('%d')}_christmas_greeting'] = state_christmas_greeting
#                 return 0
#             if state_christmas_greeting == 0:
#                 state_christmas_greeting = 1
#                 state_toml[
#                     f'{yyyy}_{get_pk_time('%m')}_{get_pk_time('%d')}_christmas_greeting'] = state_yearly_greeting
#                 return 1
#             if state_yearly_greeting == 1:
