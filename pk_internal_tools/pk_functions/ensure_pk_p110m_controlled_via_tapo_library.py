...
def ensure_pk_p110m_controlled_via_tapo_library(
    ...
    import os
    import tempfile
    from pk_internal_tools.pk_objects.pk_directories import D_PK_P110m_TO_DEPLOY
    from pk_internal_tools.pk_functions.ensure_paused import ensure_paused
    from pk_internal_tools.pk_functions.get_caller_name import get_caller_name

    if action not in ("on", "off", "toggle", "info"):
        logging.error(f"지원하지 않는 액션입니다: {action} (지원: on, off, toggle, info)")
        return False
    ...
    if not host:
        logging.error("P110M IP 주소가 입력되지 않았습니다.")
        return False
    if not username:
        username = ensure_env_var_completed(
    ...
    if not username:
        logging.error("Tapo 계정 이메일이 입력되지 않았습니다.")
        return False

    if not password:
        password = ensure_env_var_completed(
            key_name="TAPO_PW",
            func_n=func_n,
    ...
    if not password:
        logging.error("Tapo 계정 비밀번호가 입력되지 않았습니다.")
        return False

    # Xavier를 통해 P110M 제어 (Xavier가 P110M과 같은 네트워크에 있어야 함)
    logging.info(f"Xavier를 통해 P110M 제어 시작 (tapo): action={action}, host={host}")
    ...
    try:
        ...
        from pk_internal_tools.pk_objects.pk_modes import (
            PkModesForDemo,
        )
        ...
        if _cached_controller is None:
            ...
            _cached_controller = PkRemoteTargetEngine(
        ...
        if not _connection_verified or _cached_p110m_host != host:
            logging.info("Xavier에서 P110M 연결 테스트 중...")
            ...
            if exit_code != 0:
                logging.warning("Xavier에서 P110M으로 ping 실패. 네트워크 연결을 확인하세요.")
                logging.info("Xavier를 P110M과 같은 Wi-Fi 네트워크에 연결하세요.")
                _connection_verified = False
                _cached_p110m_host = None
            else:
                logging.info("Xavier에서 P110M 연결 확인됨")
                _connection_verified = True
                _cached_p110m_host = host
        else:
            logging.info("P110M 연결 확인 상태 재사용 (이미 확인됨)")

        # Xavier에 tapo 라이브러리 설치 확인
        logging.info("Xavier에 tapo 라이브러리 설치 확인 중...")
        stdout, stderr, exit_code = controller.ensure_command_to_remote_target(
        ...
        if exit_code != 0:
            logging.info("Xavier에 tapo 라이브러리 설치 중...")
            stdout, stderr, exit_code = controller.ensure_command_to_remote_target(
            ...
            if exit_code != 0:
                logging.error("Xavier에 tapo 라이브러리 설치 실패")
                return False
            logging.info("Xavier에 tapo 라이브러리 설치 완료")
        else:
            logging.info("Xavier에 tapo 라이브러리 이미 설치됨")

        # Remote script 템플릿 파일 읽기
        remote_script_template_file = D_PK_P110m_TO_DEPLOY / "__main__.py"
        remote_script_template = get_str_from_f(f=str(remote_script_template_file))

        if not remote_script_template:
            logging.error(f"remote script template file is not readable: {remote_script_template_file}")
            return False
        ...
        try:
            ...
            # Xavier에 스크립트 전송
            logging.info("P110M 제어 스크립트를 Xavier에 전송 중...")
            ...
            # Xavier에서 스크립트 실행
            logging.info("Xavier에서 P110M 제어 스크립트 실행 중...")
            stdout, stderr, exit_code = controller.ensure_command_to_remote_target(
            ...
            if exit_code == 0:
                logging.info(f"✅ P110M 제어 성공: {action}")
                ...
                logging.error(f"P110M 제어 실패: {action}")
                if stderr:
                    for line in stderr:
        ...
        logging.error(f"Xavier를 통한 P110M 제어 중 예외 발생: {e}", exc_info=True)
        return False
