...
def ensure_p110m_on():
    """
        TODO: Write docstring for ensure_p110m_on.
    """
    try:
        import logging
        import traceback
        ...
        from pk_internal_tools.pk_functions.ensure_p110m_controlled_via_tapo_library import (
            ensure_p110m_controlled_via_tapo_library,
        )
        from pk_internal_tools.pk_functions.ensure_seconds_measured import ensure_seconds_measured
        ...
        history_reset = True
        wireless_task = control_p110m_via_tapo_library_on_xavier
        try:

            # 액션 선택
            logging.info("스마트 플러그에 수행할 액션을 사용자에게 요청합니다.")
            if QC_MODE:
                p110m_action = "on" # "on", "off", "toggle", "info"
            else:
                p110m_action = "on"
            ...
            # P110M 제어 실행
            success = ensure_p110m_controlled_via_tapo_library(action=p110m_action)
