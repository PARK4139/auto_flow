...
def fix_git_push_fail_for_large_file():
    ...
    logging.info(f"{PK_UNDERLINE}\n{PK_ANSI_COLOR_MAP['BRIGHT_RED']}⚠️ 경고: 이 작업은 Git 히스토리를 영구적으로 변경합니다!{PK_ANSI_COLOR_MAP['RESET']}")
    logging.info(f"{PK_ANSI_COLOR_MAP['BRIGHT_YELLOW']}  -  공동 작업 중인 경우 팀원들에게 매우 큰 영향을 줄 수 있습니다.")
    logging.info(f"  -  실행 전에 반드시 현재 Git 저장소를 백업하십시오.")
    logging.info(f"  -  복구 불가능한 변경이 발생할 수 있습니다.{PK_ANSI_COLOR_MAP['RESET']}")
