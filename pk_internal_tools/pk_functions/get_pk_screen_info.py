    ...
    def __repr__(self):
        try:
            if self.monitors and isinstance(self.monitors, list):
                ...
                return (
                    f"Failed to detect {PkColors.RED}monitors{PkColors.RESET}.\n"
                    f"{PkColors.BRIGHT_CYAN}Debug Info: "
                    f"monitors = {self.monitors}, type = {type(self.monitors)}"
                    f"{PkColors.RESET}"
                )
        except Exception as e:
            return f"{PkColors.RED}[PkScreenInfo __repr__ Error]{PkColors.RESET} {e}"
