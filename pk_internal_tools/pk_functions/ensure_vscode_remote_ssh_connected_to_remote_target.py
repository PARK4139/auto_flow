...
def ensure_vscode_remote_ssh_connected_to_remote_target(
    remote_target_host: str = "remote_target",
    ...
    Args:
        remote_target_host: SSH config에 정의된 호스트 이름 (기본값: "remote_target")
                    또는 IP 주소 형식 (예: "pk@192.168.1.100")
        remote_target_ip: remote_target_ip 주소 (remote_target_host가 IP 주소 형식이 아닌 경우 사용)
        remote_target_user: Xavier 사용자명 (remote_target_host가 IP 주소 형식이 아닌 경우 사용)
    ...
    try:
        ...
        logger.info(f"VSCode Remote SSH 연결 자동화 시작 (호스트: {remote_target_host})")
        ...
        # 3. 연결할 호스트 이름 결정
        # remote_target_host가 IP 주소 형식이면 그대로 사용, 아니면 ssh config에 정의된 호스트 이름 사용
        connect_host = remote_target_host
        if remote_target_ip and remote_target_user:
            # IP 주소와 사용자명이 제공된 경우, 호스트 이름 대신 사용자@IP 형식 사용
            # 단, remote_target_host가 이미 IP 주소 형식이면 그대로 사용
            if "@" not in remote_target_host and not remote_target_host.replace(".", "").replace(":", "").isdigit():
                connect_host = f"{remote_target_user}@{remote_target_ip}"
                logger.debug(f"  호스트 이름 구성: {connect_host} (사용자: {remote_target_user}, IP: {remote_target_ip})")
            else:
