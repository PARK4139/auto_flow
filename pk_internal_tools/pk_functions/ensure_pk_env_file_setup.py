...
from pk_internal_tools.pk_objects.pk_files import F_ENV
...
def ensure_pk_env_file_setup() -> Path:
    ...
    try:
        ...
        # 2-1. 'D_PK_ROOT' 환경 변수 우선 확인
        selected_project_root = os.environ.get('D_PK_ROOT')

        if selected_project_root and Path(selected_project_root).exists():
            logging.info(f"환경 변수 'D_PK_ROOT'를 사용합니다: {selected_project_root}")
        else:
            # 2-2. 환경 변수가 없거나 유효하지 않으면, 기존 로직 (후보 탐색 및 사용자 선택) 수행
            logging.debug("'D_PK_ROOT' 환경 변수가 없어, 프로젝트 루트를 자동으로 탐색합니다.")
            ...
            if external_project_candidates:
                ...
                if len(external_project_candidates) > 1:
                    ...
                    selected_project_root = ensure_value_completed(
        ...
        if selected_project_root:
            os.environ['D_PK_ROOT'] = str(selected_project_root)
            dotenv_path = F_ENV
