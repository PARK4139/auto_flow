...
@ensure_seconds_measured
def ensure_p110m_controlled_via_tapo_library(
        action: str,
        *,
    ...
    try:
        ...
        remote_script_template_file = (
                d_pk_internal_tools
                / "pk_functions"
                / "pk_remote_scripts"
                / "ensure_p110m_controlled_via_tapo_library_remote.py"
        )
        remote_script_template = get_str_from_f(f=str(remote_script_template_file))

        if not remote_script_template:
            logger.error(f"Remote script 템플릿 파일을 읽을 수 없습니다: {remote_script_template_file}")
            return False
        ...
        remote_script_path = "/tmp/ensure_p110m_controlled_via_tapo_library.py"
