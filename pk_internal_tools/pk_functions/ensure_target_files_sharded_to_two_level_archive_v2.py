...
def _init_logging():
    ...
    try:
        ...
        mod = importlib.import_module("ensure_pk_log_initialized")
        if hasattr(mod, "ensure_pk_log_initialized"):
            mod.ensure_pk_log_initialized(__file__)
            return
    except Exception:
        pass
    import logging
...
def ensure_target_files_sharded_to_two_level_archive_v2(
    ...
    if long_names:
        logging.warning(f"Detected {len(long_names)} long filename(s) (>= {max_name_len}) — SKIP them.")
