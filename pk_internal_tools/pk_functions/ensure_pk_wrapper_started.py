...
def ensure_pk_wrapper_started(pk_wrapper_files=None, mode_window_front=False):
    import logging
    import os
    import subprocess
    import time
    ...
    logging.info(f"ensure_pk_wrapper_started: 시작")

    if pk_wrapper_files is None:
        t1 = time.time()
        pk_wrapper_files = get_cached_pk_wrappers_path(d_pk_wrappers)
    ...
    logging.info(f"F_VENV_PYTHON_EXE: {F_VENV_PYTHON_EXE}")
    logging.info(f"F_VENV_PYTHON_EXE exists: {os.path.exists(str(F_VENV_PYTHON_EXE))}")
    ...
    # Path 객체를 문자열로 변환
    venv_python_str = str(F_VENV_PYTHON_EXE)
