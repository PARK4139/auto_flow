...
def ensure_target_file_controlled(
        option_to_search: str = None,  # d_working is replaced by option_to_search
    ...
    각 파일에 대해 실행, 부모열기 등의 제어 옵션을 제공합니다.
    이 과정은 사용자가 명시적으로 종료하기 전까지 반복됩니다.
    테스트를 위해 choice, file_type_option, display_format_option를 인자로 받을 수 있습니다.
    """
    ...
    if option_to_search is None:
        current_pwd = get_pwd_in_python()
        options_to_search = [
            f"{current_pwd}(현재 경로)",
            "DB 기반 전체 경로"
        ]
        option_to_search = ensure_value_completed(
    ...
    if file_name_pattern_option is None:
        ...
        file_name_pattern_option = ensure_values_completed(
            key_name="file pattern to apply",
            func_n=func_n,
    ...
    if option_to_search == "DB 기반 전체 경로":
        initial_fzf_input_source = "DB"
    else:
        user_path_str = str(option_to_search).replace("(현재 경로)", "").strip()
        ...
        try:
            ...
            if display_format_option == "파일명만":
                for file_path in file_options:
                    ...
                    files_to_display.append(file_name)
            ...
            losslesscut_play_mode_option = _get_losslesscut_play_mode_option(option_to_search)
            ...
            for idx, file_path in enumerate(selected_files):
                if file_path:
                    ...
                    result = _execute_target_file_action(target_path=str(target_file_path), choice=option_to_search, losslesscut_play_mode_option=losslesscut_play_mode_option)
