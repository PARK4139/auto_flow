        ...
        try:
            ...
            if os_type == "Windows":
                ...
                try:
                    ...
                    print(f"[{PkTexts.OS_PATH_USER_CLEANED}] {PkColors.CYAN}제거된중복={len(path_list) - len(unique_paths)}개 {PkColors.RESET}")
                    
                    # 환경변수 변경 알림
                    try:
                        subprocess.run(["setx", "PATH", new_path], check=True)
                        print(f"[{PkTexts.OS_PATH_CHANGE_NOTIFICATION_SENT}] {PkColors.CYAN}상태=성공 {PkColors.RESET}")
                    except subprocess.CalledProcessError:
                        print(f"[{PkTexts.OS_PATH_CHANGE_NOTIFICATION_FAILED}] {PkColors.YELLOW}상태=실패 {PkColors.RESET}")
                        
                except Exception as e:
                    print(f"[{PkTexts.OS_PATH_CHANGE_NOTIFICATION_ERROR}] {PkColors.RED}오류={e} {PkColors.RESET}")
                ...
                for config_file in shell_configs:
                    if os.path.exists(config_file):
                        try:
                            ...
                            print(f"[{PkTexts.OS_PATH_USER_CLEANED}] {PkColors.CYAN}설정파일={config_file} {PkColors.RESET}")
                            break
                            
                        except Exception as e:
                            print(f"[{PkTexts.OS_PATH_CHANGE_NOTIFICATION_ERROR}] {PkColors.RED}파일={config_file} 오류={e} {PkColors.RESET}")
            
            return True
            
        except Exception as e:
            print(f"[{PkTexts.OS_PATH_CLEAN_FAILED}] {PkColors.RED}오류={e} {PkColors.RESET}")
            return False
