...
def ensure_remote_target_controlled():
    import logging
    import traceback
    ...
    try:
        ...
        # n. 무선 작업 목록 정의
        control_terminal_on_remote_target = "CONTROL TERMINAL ON XAVIER"  # WIRELESS PROGRAMMING
        control_p110m_via_tapo_library_on_remote_target = "CONTROL P110M VIA TAPO LIBRARY ON XAVIER"
        control_p110m_via_python_kasa_library_on_remote_target = "CONTROL P110M VIA PYTHON-KASA LIBRARY(TAPO LOCAL API) ON XAVIER"
        control_p110m_via_ha_on_remote_target = "CONTROL P110M VIA HOME ASSISTANT ON XAVIER"
        control_p110m_monitor_on_remote_target = "CONTROL P110M MONITOR ON XAVIER"
        execute_pk_interesting_data_dashboard_and_control_sever_on_remote_target = "EXECUTE PK INTERESTING DATA DASHBOARD AND CONTROL SEVER ON XAVIER"
        arduino_code_dev_env_on_remote_target = "ONBOARD ARDUINO DEV ENVIRONMENT ON XAVIER"
        control_tv_server_on_remote_target = "CONTROL TV SERVER ON XAVIER"
        execute_api_server_on_remote_target = "EXECUTE API SERVER ON XAVIER"
        execute_kiria_server_on_remote_target = "EXECUTE KIRI SERVER ON XAVIER"  # TBD : Xavier 와 PK_QCY_H3_ANC_HEADSET를 자동연결 via Bluetooth # Bluetooth USB dongle 별도 필요하여 고민 중, # TODO : 실시간 앵무새
        develop_arduino_ota_environment_on_remote_target = "DEVELOP ARDUINO OTA ENVIRONMENT ON XAVIER"
        develop_arduino_code_on_remote_target = "DEVELOP ARDUINO CODE ON XAVIER"  # WIRELESS PROGRAMMING
        control_remote_target_via_chrome_remote_desktop_on_host = "CONTROL WIRELESS TARGET VIA CHROME REMOTE DESKTOP ON HOST"
        remote_tasks = [
            control_tv_server_on_remote_target,
            execute_api_server_on_remote_target,
            execute_pk_interesting_data_dashboard_and_control_sever_on_remote_target,  # 여기에 control_p110m_monitor_on_remote_target 기능을 통합 pk p110m monitor health check 죽었으면, 죽은사유로깅 && 재실행
            arduino_code_dev_env_on_remote_target,
            develop_arduino_code_on_remote_target,
            develop_arduino_ota_environment_on_remote_target,
            execute_kiria_server_on_remote_target,
            # control_p110m_via_python_kasa_library_on_remote_target, # control_p110m_via_tapo_library_on_remote_target 로 대체 가능하여, 개발필요성 떨어져 임시주석
            # control_p110m_via_ha_on_remote_target,  # control_p110m_via_tapo_library_on_remote_target 로 대체 가능하여, 개발필요성 떨어져 임시주석, 개발필요시 주석해제 예정
            control_remote_target_via_chrome_remote_desktop_on_host,  # succeeded, TODO pk_asus | pk_renova | pk_huvitz_gram 등등 fzf 로 선택하도록
            control_terminal_on_remote_target,  # succeeded
            control_p110m_via_tapo_library_on_remote_target,  # succeeded, 전화번호 말고 이메일형식으로 2차 인증없이
        ]
        history_reset = True

        # n. 사용자에게 작업 선택받기
        remote_task = None
        while not remote_task:
            logging.info("수행할 무선 제어 작업을 사용자에게 요청합니다.")

            # QC_MODE와 상관없이 동일한 fzf 호출 사용
            remote_task = ensure_value_completed(
                key_name="remote_task",
                func_n=func_n,
                options=remote_tasks,
                guide_text="수행할 무선 제어 작업을 선택하세요:",
                history_reset=history_reset
            )

            # fzf에서 아무것도 선택하지 않으면 empty string('') 또는 '[]' 문자열이 반환될 수 있음
            if not remote_task or remote_task == '[]':
                remote_task = None  # 루프를 다시 실행하기 위해 None으로 리셋
        ...
        logging.info(f"선택된 작업: {remote_task}")

        # n. 선택된 작업에 따라 로직 분기
        if remote_task == control_terminal_on_remote_target:
            ensure_remote_target_terminal_opened_via_ssh_like_person()

        if remote_task == control_p110m_via_ha_on_remote_target:
            logging.info(f"'{control_p110m_via_ha_on_remote_target}' 작업을 시작합니다.")

            try:
                # 기존 controller 재사용 또는 새로 생성
                if shared_controller is None:
                    logging.info("remote_target 연결 생성 중...")
                    shared_controller = PkRemoteTargetEngine(identifier=PkDevice.jetson_agx_xavier, setup_op=PkModesForDemo.REMOTE_TARGET)
                    controller_initialized = True
                else:
                    logging.info("기존 remote_target 연결 재사용")
                ...
                if not ensure_home_assistant_ready_on_target(controller):
                    ...
                    return
                p110m_manager = PkP110mController(remote_target_controller=controller)
                ...
                try:
                    stdout, _, exit_status = controller.ensure_command_to_remote_target(
                ...
                def _diagnose_home_assistant_access(controller):
                    ...
                    for desc, cmd, needs_sudo in diagnostics:
                        try:
                            ...
                            stdout, stderr, exit_status = controller.ensure_command_to_remote_target(
                                cmd=cmd,
                                timeout_seconds=20,
                                use_sudo=needs_sudo,
                            )
                ...
                if not ha_token:
                    ...
                    remote_target_ip = getattr(controller.remote_target, "ip", None) or getattr(controller.remote_target, "hostname", None)
                    if remote_target_ip:
                        try:
                            dom_url = f"http://{remote_target_ip}:8123/onboarding.html"
        ...
        elif remote_task == control_p110m_via_python_kasa_library_on_remote_target:
            logging.info(f"'{control_p110m_via_python_kasa_library_on_remote_target}' 작업을 시작합니다.")
        ...
        elif remote_task == control_p110m_via_tapo_library_on_remote_target:
            logging.info(f"'{control_p110m_via_tapo_library_on_remote_target}' 작업을 시작합니다.")
        ...
        elif remote_task == control_p110m_monitor_on_remote_target:
            logging.info(f"'{control_p110m_monitor_on_remote_target}' 작업을 시작합니다.")
            ...
            try:
                ...
                try:
                    # Xavier에 스크립트 전송
                    remote_script_path = "/tmp/ensure_pk_p110m_monitor_on_remote_target.py"
                    ...
                    if exit_code == 0:
                        ...
                        logging.info("모니터링 중지: Xavier에서 'pkill -f ensure_pk_p110m_monitor_on_remote_target.py'")
                    else:
        ...
        elif remote_task == execute_pk_interesting_data_dashboard_and_control_sever_on_remote_target:
            logging.info(f"'{execute_pk_interesting_data_dashboard_and_control_sever_on_remote_target}' 작업을 시작합니다.")

            try:
                from pk_internal_tools.pk_functions.ensure_pk_web_server_executed_on_remote_target_on_remote_target import (
                    ensure_pk_web_server_executed_on_remote_target_on_remote_target,
                )
                ...
                # 멀티 데이터 대시보드 서버 시작
                success = ensure_pk_web_server_executed_on_remote_target_on_remote_target(
                    host="0.0.0.0",
                    port=port,
                ...
                if success:
                    ...
                    logging.info("📊 접속 URL: http://%s:%d", remote_target_ip, port)
                    logging.info("서버 로그 확인: ssh로 Xavier 접속 후 'tail -f /tmp/multi_data_pk_dashboard_server.log'")
                    logging.info("서버 중지: Xavier에서 'pkill -f ensure_pk_web_server_on_remote_target.py'")
                else:
        ...
        elif remote_task == arduino_code_dev_env_on_remote_target:
            logging.info(f"'{arduino_code_dev_env_on_remote_target}' 작업을 시작합니다.")
            ...
            try:
                ...
                controller = shared_controller
                remote_target_ip = getattr(controller.remote_target, "ip", None) or getattr(controller.remote_target, "hostname", None)
                remote_target_user = getattr(controller.remote_target, "user_n", None) or "pk"
                remote_target_pw = getattr(controller.remote_target, "pw", None)
                ...
                if success:
                    ...
                    logging.info("2. '%s@%s' 입력", remote_target_user, remote_target_ip)
        ...
        elif remote_task == control_tv_server_on_remote_target:
            logging.info(f"'{control_tv_server_on_remote_target}' 작업을 시작합니다.")
            ...
            try:
                ...
                from pk_internal_tools.pk_functions.ensure_tv_controlled_via_ha_on_remote_target import PkTvController

                logging.info("TV 제어 컨트롤러를 초기화합니다...")
                tv_manager = PkTvController(remote_target_controller=controller)
                ...
                if not ha_token:
                    try:
                        ...
                        env_var_id = get_pk_env_var_id("HA_TOKEN", func_n)
                ...
                if not ha_token:
                    try:
                        ...
                        remote_target_ip = getattr(controller.remote_target, "ip", None) or getattr(controller.remote_target, "hostname", None)
                        ha_url = f"http://{remote_target_ip}:8123" if remote_target_ip else "http://localhost:8123"
        ...
        elif remote_task == execute_api_server_on_remote_target:
            logging.info(f"'{execute_api_server_on_remote_target}' 작업을 시작합니다.")

            try:
                from pk_internal_tools.pk_functions.ensure_api_server_started_on_remote_target import (
                    ensure_api_server_started_on_remote_target,
                )
                ...
                # API 서버 시작
                success = ensure_api_server_started_on_remote_target(
                    host="0.0.0.0",
                    port=port,
                    remote_target_ip=remote_target_ip,
                )
                ...
                if success:
                    ...
                    logging.info("🌐 접속 URL: http://%s:%d", remote_target_ip, port)
                    logging.info("📋 API 문서: http://%s:%d/docs", remote_target_ip, port)
        ...
        elif remote_task == execute_kiria_server_on_remote_target:
            logging.info(f"'{execute_kiria_server_on_remote_target}' 작업을 시작합니다.")

            try:
                from pk_internal_tools.pk_functions.ensure_kiria_server_started_on_remote_target import (
                    ensure_kiria_server_started_on_remote_target,
                )
                ...
                # kiri 서버 시작
                success = ensure_kiria_server_started_on_remote_target(
                    remote_target_ip=remote_target_ip,
                    remote_target_user=remote_target_user,
                    remote_target_pw=remote_target_pw,
                ...
                if success:
                    ...
                    logging.info("서버 로그 확인: ssh로 Xavier 접속 후 'tail -f /tmp/kiria_server.log'")
                    logging.info("서버 중지: Xavier에서 'pkill -f ensure_kiria_server_on_remote_target.py'")
                else:
        ...
        elif remote_task == develop_arduino_code_on_remote_target:
            logging.info(f"'{develop_arduino_code_on_remote_target}' 작업을 시작합니다.")

            try:
                from pk_internal_tools.pk_functions.ensure_development_code_uploaded_from_remote_target_to_arduino_wireless import (
                    ensure_development_code_uploaded_from_remote_target_to_arduino_wireless,
                )
                ...
                # Wireless 코드 업로드
                success = ensure_development_code_uploaded_from_remote_target_to_arduino_wireless(
                    remote_target_ip=remote_target_ip,
                    remote_target_user=remote_target_user,
                    remote_target_pw=remote_target_pw,
        ...
        elif remote_task == develop_arduino_ota_environment_on_remote_target:
            logging.info(f"'{develop_arduino_ota_environment_on_remote_target}' 작업을 시작합니다.")

            try:
                from pk_internal_tools.pk_functions.ensure_arduino_ota_development_environment_on_remote_target import (
                    ensure_arduino_ota_development_environment_on_remote_target,
                )
                ...
                success = ensure_arduino_ota_development_environment_on_remote_target(
                    remote_target_ip=remote_target_ip,
                    remote_target_user=remote_target_user,
                    remote_target_pw=remote_target_pw,
                    esp8266_ip=None,  # 사용자 입력받기
        ...
        elif remote_task == control_remote_target_via_chrome_remote_desktop_on_host:
            logging.info(f"'{control_remote_target_via_chrome_remote_desktop_on_host}' 작업을 시작합니다.")
