...
def get_backup_directory_by_os():
    ...
    if os_type == "wsl" or os_type == "linux":
        # Linux/WSL 환경
        return os.path.join(os.path.expanduser("~"), "Downloads", "pk_system", ".pk_system")
    elif os_type == "windows":
        # Windows 환경
        return os.path.join(os.path.expanduser("~"), "Downloads", "pk_system", ".pk_system")
    else:
...
def ensure_bashrc_zshrc_backed_up():
    ...
    try:
        ...
        print(f"[{PkTexts.BACKUP_OS_DETECTED}] {PK_ANSI_COLOR_MAP['CYAN']}OS={os_type} {PK_ANSI_COLOR_MAP['RESET']}")
        ...
        for file_path in files_to_backup:
            try:
                ...
                print(f"[{PkTexts.BACKUP_COMPLETE}] {PK_ANSI_COLOR_MAP['CYAN']}파일={filename} 백업={backup_filename} {PK_ANSI_COLOR_MAP['RESET']}")
                
            except Exception as e:
                print(f"[{PkTexts.BACKUP_FAILED}] {PK_ANSI_COLOR_MAP['RED']}파일={file_path} 오류={str(e)} {PK_ANSI_COLOR_MAP['RESET']}")
            except FileNotFoundError:
                print(f"[{PkTexts.BACKUP_FILE_NOT_FOUND}] {PK_ANSI_COLOR_MAP['YELLOW']}파일={file_path} {PK_ANSI_COLOR_MAP['RESET']}")
        
        # 결과 요약
        if backed_up_files:
            print(f"[{PkTexts.BACKUP_TOTAL_COMPLETE}] {PK_ANSI_COLOR_MAP['CYAN']}총파일수={len(backed_up_files)}개 {PK_ANSI_COLOR_MAP['RESET']}")
            return True
        else:
            print(f"[{PkTexts.BACKUP_NO_FILES}]")
            return False
        ...
        print(f"[{PkTexts.BACKUP_FAILED}] {PK_ANSI_COLOR_MAP['RED']}오류={e} {PK_ANSI_COLOR_MAP['RESET']}")
        return False
