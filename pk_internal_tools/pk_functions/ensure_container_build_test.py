...
def ensure_container_build_test():
    ...
    try:
        ...
        print(f"[{PkTexts.CONTAINER_PROJECT_PATH_CHECK}] {PK_ANSI_COLOR_MAP['CYAN']}경로={project_path} {PK_ANSI_COLOR_MAP['RESET']}")
        ...
        for service_name in services:
            print(f"[{PkTexts.CONTAINER_SERVICE_BUILDING}] {PK_ANSI_COLOR_MAP['YELLOW']}서비스={service_name} {PK_ANSI_COLOR_MAP['RESET']}")
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    # 서비스 시작
                    print(f"[{PkTexts.CONTAINER_SERVICE_STARTING}] {PK_ANSI_COLOR_MAP['YELLOW']}서비스={service_name} {PK_ANSI_COLOR_MAP['RESET']}")
        ...
        print(f"[{PkTexts.CONTAINER_BUILD_TEST_SUCCESS}] {PK_ANSI_COLOR_MAP['CYAN']}상태=완료 {PK_ANSI_COLOR_MAP['RESET']}")
        
        # 유용한 명령어 출력
        print(f"[{PkTexts.CONTAINER_USEFUL_COMMANDS}] {PK_ANSI_COLOR_MAP['CYAN']}명령어=도움말 {PK_ANSI_COLOR_MAP['RESET']}")
        print("유용한 명령어:")
        ...
        print(f"[{PkTexts.CONTAINER_TEST_FAILED}] {PK_ANSI_COLOR_MAP['RED']}오류={e} {PK_ANSI_COLOR_MAP['RESET']}")
        return False
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if healthy_count == len(services):
                print(f"[{PkTexts.CONTAINER_TEST_SUCCESS}] {PK_ANSI_COLOR_MAP['CYAN']}정상서비스수={healthy_count}개 {PK_ANSI_COLOR_MAP['RESET']}")
                return True
            else:
                print(f"[{PkTexts.CONTAINER_TEST_FAILED}] {PK_ANSI_COLOR_MAP['RED']}정상서비스수={healthy_count}개 전체서비스수={len(services)}개 {PK_ANSI_COLOR_MAP['RESET']}")
                return False
        else:
            print(f"[{PkTexts.CONTAINER_TEST_FAILED}] {PK_ANSI_COLOR_MAP['RED']}컨테이너상태확인실패 {PK_ANSI_COLOR_MAP['RESET']}")
            return False
