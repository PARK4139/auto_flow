...
def ensure_pk_interesting_infos_spoken(
    ...
    # 음성으로 출력할 문장 생성
    speech_parts = []
    if flags & PkModesForGetPkInterestingInfo.DATE and data.date and data.day_of_week:
        speech_parts.append(f"오늘은 {data.date} {data.day_of_week}")
    elif flags & PkModesForGetPkInterestingInfo.DATE and data.date:
    ...
    if full_speech:
        ensure_spoken(full_speech, read_finished_wait_mode=True) # 음성이 끝날 때까지 대기
    else:
        ensure_spoken("알려드릴 정보가 없습니다.", read_finished_wait_mode=True)