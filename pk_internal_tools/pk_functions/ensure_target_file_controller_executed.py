def _ensure_target_file_action_executed(target_path: str, choice, losslesscut_play_mode_option):
    """
    ...
    if choice == "실행":
    ...
    elif choice == "부모 폴더 열기":
        parent_dir = target.parent
        logging.info(f"부모 폴더 열기: {parent_dir}")
        ...
        # pk_option
        from pk_internal_tools.pk_objects.pk_system_operation_options import PlayerSelectionMode
        from pk_internal_tools.pk_functions.ensure_value_completed import ensure_value_completed
        func_n = get_caller_name()  # 내부 호출자 함수 이름을 가져옴

        selected_selection_mode = PlayerSelectionMode[losslesscut_play_mode_option.upper()] if losslesscut_play_mode_option else PlayerSelectionMode.AUTO
        ...
        potplayer_path = F_POT_PLAYER_EXE
...
def ensure_target_file_controller_executed(

        d_working: str = None,
        choice: str = None,
        file_type_choice: str = None,
        display_format_choice: str = None
):
    """
    fzf를 사용하여 지정된 디렉토리(d_working)에서 하나 이상의 파일을 선택한 후,
    각 파일에 대해 실행, 부모 폴더 열기 등의 제어 옵션을 제공합니다.
    이 과정은 사용자가 명시적으로 종료하기 전까지 반복됩니다.
    테스트를 위해 choice, file_type_choice, display_format_choice를 인자로 받을 수 있습니다.
    """
    import logging
    import traceback
    from pathlib import Path
    ...
    if file_type_choice is None:
        file_type_options = ["모든파일", "비디오만", "이미지만", "문서만"]
        file_type_choice = ensure_value_completed(
    ...
    if file_type_choice == "비디오만":
        allowed_extensions = list(FILE_EXTENSIONS['videos'])
    elif file_type_choice == "이미지만":
        allowed_extensions = list(FILE_EXTENSIONS['images'])
    elif file_type_choice == "문서만":
        allowed_extensions = list(FILE_EXTENSIONS['documents'])

    if display_format_choice is None:
        display_format_options = ["파일명만", "파일경로전체"]
        display_format_choice = ensure_value_completed(
    ...
    if not display_format_choice:
        logging.info("출력 형태 선택이 취소되었습니다. 작업을 종료합니다.")
        return

    if choice == "DB 기반 전체 경로":
        initial_fzf_input_source = "DB"
    else:
        user_path_str = choice.replace("(현재 경로)", "").strip()
        ...
        try:
            ...
            if initial_fzf_input_source == "DB":
                ...
                if d_path.is_dir():
                    ...
                    if allowed_extensions:
                        allowed_extensions_lower = [ext.lower() for ext in allowed_extensions]
                        file_options = [f for f in all_files if Path(f).suffix.lower() in allowed_extensions_lower]
                        logging.debug(f"[{loop_count}] 직접 스캔으로 {len(all_files)}개 파일 중 {len(file_options)}개 필터링됨")
                    else:
                        file_options = all_files
                        logging.debug(f"[{loop_count}] 직접 스캔으로 {len(file_options)}개 파일 발견")
                else:
            ...
            if display_format_choice == "파일명만":
                for file_path in file_options:
                    file_name = Path(file_path).name
                    fzf_display_options.append(file_name)
                ...
                fzf_display_options = file_options

            selected_display_values = ensure_values_completed(
                key_name="파일 선택",
                func_n=func_n,
                options=fzf_display_options,
                multi_select=True
            )

            if not selected_display_values:
                logging.info("파일 선택이 취소되었습니다. 루프를 종료합니다.")
                break

            selected_files = []
            if display_format_choice == "파일명만":
                for selected_name in selected_display_values:
                ...
                selected_files = selected_display_values
            ...
            actions = ["실행", "부모 폴더 열기", "경로 복사", "Play with Losslesscut", "Play with Potplayer", "종료"]
            func_n = get_caller_name()
            choice = ensure_value_completed(
                key_name=key_name,
                func_n=func_n,
            ...
            if choice == "Play with Losslesscut":
                ...
                losslesscut_play_mode_option = ensure_value_completed(
                    key_name=f"losslesscut_play_mode_option",  # key_name을 동적으로 생성
                    func_n=func_n,
            ...
            for idx, file_path in enumerate(selected_files):
                if file_path:
                    ...
                    result = _ensure_target_file_action_executed(target_path=str(target_file_path), choice=choice, losslesscut_play_mode_option=losslesscut_play_mode_option)
