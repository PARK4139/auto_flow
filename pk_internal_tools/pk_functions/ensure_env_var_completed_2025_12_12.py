...
def ensure_env_var_completed_2025_12_12(
        key_name: str,
        func_n,
        mask_log: bool = True,
    ...
    env_var_id = get_pk_env_var_id(key_name, func_n)
    ...
    if options is None:  # Case 1. options가 없는 경우
        ...
        if not value:  # ENV에 값이 없을 때만 options에서 선택
            logging.warning(f"Environment variable '{key_name}' not found.")
            value = ensure_value_completed(key_name=key_name, options=options, func_n=func_n)
