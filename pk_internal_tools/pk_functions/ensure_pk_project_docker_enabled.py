def ensure_pk_project_docker_ran(f, dockerfile_script_list):
    ...
    ensure_command_to_remote_target(cmd=f'scp -av --delete -e "ssh -i ~/.ssh/id_ed25519" {D_PK_ROOT} {user_n}@{ip}:~/Downloads/{get_nx(D_PK_ROOT)}', **wsl_distro_config)
    # mkr. # todo
    ...
    # start docker deamon
    ensure_command_to_remote_target(cmd=f'sudo service docker start', **wsl_distro_config)
    ...
    ensure_command_to_remote_target(cmd=rf"docker build -t {f_docker_tag_n} -f ~/Downloads/{os.path.basename(d_pk_system)}/{f_nx} .",
                             **wsl_distro_config)
    std_out_str, std_err_str = ensure_command_to_remote_target(cmd=rf"ls ~/Downloads/{os.path.basename(d_pk_system)}/{f_nx}",
                                                        **wsl_distro_config)
