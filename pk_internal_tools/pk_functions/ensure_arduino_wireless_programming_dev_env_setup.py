...
def _check_esp_link_connection(controller: PkRemoteTargetEngine, esp8266_ip: Optional[str] = None) -> bool:
    """esp-link 연결 확인"""
    if not esp8266_ip:
...
def ensure_arduino_wireless_programming_dev_env_setup(
    ...
    try:
        ...
        logger.info("")
        logger.info("다음 단계:")
        logger.info("1. VSCode Remote-SSH로 remote_target 연결: pk@%s", remote_target_ip)
