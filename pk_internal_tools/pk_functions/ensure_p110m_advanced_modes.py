...
from pk_internal_tools.pk_functions.ensure_p110m_controlled_via_ha_api import (
    ensure_p110m_controlled_via_ha_api,
    get_ha_entity_state,
)
import requests
...
@ensure_seconds_measured
def ensure_p110m_sequence_control(
    ...
    *,
    ...
    예시:
        ...
        ensure_p110m_sequence_control(sequence)
    ...
    try:
        for i, step in enumerate(sorted_sequence):
            ...
            success = ensure_p110m_controlled_via_ha_api(
                action=step.action,
                entity_id=entity_id,
                ha_url=ha_url,
                ha_token=ha_token,
...
@ensure_seconds_measured
def ensure_p110m_energy_saving_mode(
            ...
            if config.power_threshold_watts is not None and current_state == "on":
                ...
                if energy_data and energy_data.get("power_watts") is not None:
                    ...
                    if current_power > config.power_threshold_watts:
                        ...
                        # 필요시 자동 끄기 (설정에 따라)
                        # ensure_p110m_controlled_via_ha_api(action="off", ...)
...
@ensure_seconds_measured
def ensure_p110m_scheduled_control(
    ...
    *,
    ...
    예시:
        ...
        ensure_p110m_scheduled_control(schedule)
...
def example_energy_saving_mode():
    ...
    ensure_p110m_energy_saving_mode(config, duration_seconds=3600)  # 1시간 동안 모니터링
