...
from pk_internal_tools.pk_objects.pk_directories import (
    ...
    D_PK_EXTERNAL_TOOLS,
    D_DESKTOP,
    D_PK_RECYCLE_BIN, D_PK_ROOT, D_AUTO_FLOW_REPO, D_USERPROFILE,D_RECYCLE_BIN
)
from pk_internal_tools.pk_objects.pk_files import (
    ...
    F_VSCODE_LNK,
    F_CLAUDE_LNK,
    F_PYCHARM_EXE, # F_PYCHARM64_EXE -> F_PYCHARM_EXE
    F_ENSURE_CMD_EXE_RAN_AS_ADMIN,
)
from pk_internal_tools.pk_objects.pk_files import F_UV_PYTHON_EXE, F_PK_ENSURE_PK_WRAPPER_STARTED_PY, F_RUN_CMD
...
def _get_pk_doskey_commands() -> dict[str, str]:
    ...
    doskey_commands = {
        # 디렉토리 이동
        f'1: cd /d "{D_DOWNLOADS}"': f'doskey 1=cd /d "{D_DOWNLOADS}"',
        f'2: cd /d "{D_PK_ROOT}"': f'doskey 2=cd /d "{D_PK_ROOT}"',
        f'3: cd /d "{D_PK_MEMO_REPO}"': f'doskey 3=cd /d "{D_PK_MEMO_REPO}"',
        f'4: cd /d "{D_PK_WORKING}"': f'doskey 4=cd /d "{D_PK_WORKING}"',
        f'5: cd /d "{D_AUTO_FLOW_REPO}"': f'doskey 5=cd /d "{D_AUTO_FLOW_REPO}"',
        f'6: cd /d "{D_USERPROFILE}"': f'doskey 6=cd /d "{D_USERPROFILE}"',
        f'7: cd /d "{D_DESKTOP}"': f'doskey 7=cd /d "{D_DESKTOP}"',
        f'8: cd /d "{D_DESKTOP}"': f'doskey 8=cd /d "{D_DESKTOP}"',
        f'9: cd /d "{D_PK_RECYCLE_BIN}"': f'doskey 9=cd /d "{D_PK_RECYCLE_BIN}"',
        f'0: cd /d "{D_RECYCLE_BIN}"': f'doskey 0=cd /d "{D_RECYCLE_BIN}"',
        ...
        f'pk: PK wrapper 실행': f'doskey pk=call "{F_RUN_CMD}"',
    ...
    # PyCharm doskey를 F_PYCHARM_EXE의 존재 여부에 따라 조건부 추가
    if F_PYCHARM_EXE:
        doskey_commands[f'pycharm: PyCharm 실행'] = f'doskey pycharm=start "" "{F_PYCHARM_EXE}" $*'
...
def ensure_pk_doskey_registered():
    ...
    try:
        ...
        selected_doskey_displays = ensure_values_completed(
