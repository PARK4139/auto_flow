...
from pk_internal_tools.pk_objects.pk_etc import PK_WHITE, PK_PINK, PK_POINTER_COLOR, PK_HL_COLOR, PK_FG_ACTIVE_COLOR, PK_HL_ACTIVE_COLOR, PK_QUERY_COLOR, PK_POINTER_TEXT, PK_SPINNER_COLOR
from pk_internal_tools.pk_objects.pk_fzf_theme import PkFzfTheme


def _get_cmd_pk_fzf_binded(cmd_list, entries, max_len=1500):
...
def _alert_command_line_length(cmd, length_limit):
    try:
        from subprocess import list2cmdline
        import logging
        from typing import List, Optional

        from pk_internal_tools.pk_functions.is_os_macos import is_os_macos
        from pk_internal_tools.pk_objects.pk_fzf_theme import PkFzfTheme
        ...
        if len(cmdline_preview) > length_limit:
            logging.warning(f"{PkTexts.WARNING} fzf command line length={len(cmdline_preview)} (>7000)")
            alert_as_gui(f"{PkTexts.WARNING} fzf command line length={len(cmdline_preview)} (>7000)")
    except Exception as e:
        pass


def get_value_from_fzf_routine(file_id, editable, options, query="", guide_text=None, fzf_theme: PkFzfTheme = PkFzfTheme(), multi_select: bool = False, custom_fzf_options: Optional[List[str]] = None):
    import os
    import subprocess
    import tempfile
    import textwrap
    ...
    try:
        ...
        if query and len(query) > SAFE_QUERY_MAX:
            logging.warning(f"query too long ({{len(query)}}), truncated to {SAFE_QUERY_MAX}")
...
{user_input_guide_text}

# 단축키 {PkTexts.TUTORIAL}
{ensure_value_complete_shortcut_guide_text}
        ''').strip()
        if guide_text is None:
            footer_text = footer_guide
            footer_text_color = PK_PINK
        else:
        ...
        if custom_fzf_options:  # Append custom options
            cmd.extend(custom_fzf_options)

        cmd += [
            "--no-mouse",
            f"--prompt={pk_label}={PK_BLANK}{PK_BLANK}",
            f"--pointer={PK_POINTER_TEXT}",
            f"--color=query:{PK_QUERY_COLOR},spinner:{PK_SPINNER_COLOR},prompt:{PK_WHITE},pointer:{PK_POINTER_COLOR},hl:{PK_HL_COLOR},hl+:{PK_HL_ACTIVE_COLOR},fg+:{PK_FG_ACTIVE_COLOR},footer:{footer_text_color}",
            "--footer", footer_text,
        ...
        cmd = _get_cmd_pk_fzf_binded(cmd, pk_fzf_bind_options)

        _alert_command_line_length(cmd, 7000)
