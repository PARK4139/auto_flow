...
def ensure_pk_yeelight_toggle(
    ...
    """
    Orchestrates the full flow to TOGGLE the state of a Yeelight device:
    provisioning, token retrieval, IP discovery, optional developer mode setup,
    and finally, setting the state to "toggle".
    ...
    Returns:
        True if the Yeelight is successfully toggled, False otherwise.
    """
    func_n = get_caller_name()

    logging.info(PK_UNDERLINE)
    logging.info(f"{PkColors.BRIGHT_CYAN}YeeLight TOGGLE 제어 흐름 시작{PkColors.RESET}")
    logging.info(PK_UNDERLINE)
    ...
    try:
        ...
        if controller is None:
            logging.error("Xavier 원격 컨트롤러 확보 실패. YeeLight TOGGLE 제어를 중단합니다.")
            return False
        ...
        # 5. State Change (TOGGLE)
        logging.info(f"YeeLight ({yeelight_ip}) 상태를 'toggle'으로 변경 시도합니다...")
        ...
        if control_success:
            logging.info(f"✅ YeeLight '{yeelight_ip}' 상태를 'toggle'으로 성공적으로 변경했습니다.")
            return True
        else:
            logging.error(f"YeeLight '{yeelight_ip}' 상태를 'toggle'으로 변경하는 데 실패했습니다.")
            return False

    except Exception as e:
        logging.error(f"YeeLight TOGGLE 제어 흐름 중 오류 발생: {e}", exc_info=True)
        ensure_debugged_verbose(traceback, e)
        return False
    finally:
        logging.info(PK_UNDERLINE)
        logging.info(f"{PkColors.BRIGHT_CYAN}YeeLight TOGGLE 제어 흐름 종료{PkColors.RESET}")
        logging.info(PK_UNDERLINE)
