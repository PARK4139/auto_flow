...
def _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text, new_text, d_checkout):
    from pk_internal_tools.pk_functions.ensure_target_file_names_and_target_file_texts_replaced_advanced import ensure_target_file_names_and_target_file_texts_replaced_advanced
    ...
    ensure_target_file_names_and_target_file_texts_replaced_advanced(
        d_target=d_checkout,
        old_text=old_text,
        new_text=new_text,
...
def ensure_project_published(*, d_project_to_publish: Path, d_destination: Path, f_gitignore_base: Path, blacklist: list[Path], publishlist: list[Path]):
    ...
    from pk_internal_tools.pk_objects.pk_urls import URL_GIT_HUB_AUTO_FLOW_GIT
    ...
    try:
        ...
        # git clone pk_clone
        repository_url = URL_GIT_HUB_AUTO_FLOW_GIT
        ...
        # replace project file contents text and file_name text
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_external_tools", new_text="resources", d_checkout=d_checkout)
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_internal_tools", new_text="sources", d_checkout=d_checkout)
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_wrappers", new_text="wrappers", d_checkout=d_checkout)
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_objects", new_text="objects", d_checkout=d_checkout)
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_functions", new_text="functions", d_checkout=d_checkout)
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_info", new_text="system_info", d_checkout=d_checkout)
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_os_layer_resources", new_text="system_resources", d_checkout=d_checkout)
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_sounds", new_text="system_sounds", d_checkout=d_checkout)
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_logs", new_text="logs", d_checkout=d_checkout)
        # _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_deprecated.toml", new_text="project_config.toml", d_checkout=d_checkout)
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text=f"pk_qc_mode_toml", new_text="project_config", d_checkout=d_checkout)
        ensure_spoken(get_easy_speakable_text(f'파일트리 명칭 변경완료'))

        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text="pk_system", new_text=get_nx(d_destination), d_checkout=d_checkout)
        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text="pk_system".upper(), new_text=get_nx(d_destination).upper(), d_checkout=d_checkout)
        ensure_spoken(get_easy_speakable_text(f'프로젝트명 변경완료'))

        _ensure_replace_file_name_and_file_contents_replaced_advanced(old_text="wjdgn", new_text='pk_security_literal', d_checkout=d_checkout)
        ...
        ensure_paused("깃 푸쉬 성공")

        ensure_target_opened_advanced(URL_GIT_HUB_AUTO_FLOW_GIT)
        ensure_pnx_opened_by_ext(d_checkout)
        return True
