...
def get_pk_language():
    ...
    try:
        ...
        is_pk_initial_launched = db.get_state_value(db_id=db.get_db_id(key_hint1, func_n))
        ...
        if is_pk_initial_launched is True:
            ...
            pk_language = get_values_from_historical_file_routine(
            ...
            if pk_language is None:
                logging.debug(f"{key_name} is missing. Re-configuring...")
                pk_language = get_values_from_historical_database_routine(
        ...
        logging.debug(f"{func_n} {key_name} = {pk_language}")
        return pk_language
    except Exception as e:
        logging.warning(f"get_pk_language 오류: {e}")
        return "en"  # 기본값은 "en" 코드로 통일
