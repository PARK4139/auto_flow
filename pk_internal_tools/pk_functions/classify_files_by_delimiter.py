...
def _create_directory_structure_from_filename(base_dir: Path, filename: str, delimiter: str) -> Path:
    """
    Splits a filename by a delimiter and creates a nested directory structure.
    The directory path is built from all parts of the filename stem except the last one.
    """
    name_parts = Path(filename).stem.split(delimiter)
...
def classify_files_by_delimiter(source_files: List[str], destination_dir: str, delimiter: str = '_'):
    ...
    Args:
        ...
        delimiter (str, optional): The character to split filenames by. Defaults to '_'.
    """
    ...
    for source_file_path in source_files:
        ...
        try:
            target_dir = _create_directory_structure_from_filename(
