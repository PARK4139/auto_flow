def ensure_target_files_classified_by_ngram(d_working, token_splitter_pattern, min_support, max_n, files_to_organize, allowed_extension_tuple=None):
    ...
    from pk_internal_tools.pk_functions.get_filename_without_timestamp import get_filename_without_timestamp
    ...
    ans = ensure_value_completed_2025_11_11(key_name="위와 같이 파일을 분류할까요? (o/x):", options=["o", "x"], func_n=func_n)
    ans = ans.strip()
    ans = ans.lower()
        ...
        for category, files in categorized_f_dict.items():
            ...
            for f_nx in files:
                ...
                if os.path.exists(dst):
                    ...
                    original_filename = get_filename_without_timestamp(f_nx, d_working)
