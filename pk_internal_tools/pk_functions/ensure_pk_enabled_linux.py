    ...
    def install_uv(self, max_retry: int = 2) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"[{PkTexts.LINUX_UV_ALREADY_INSTALLED}] {PkColors.CYAN}버전={result.stdout.strip()} {PkColors.RESET}")
                return True
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        result = subprocess.run([self.f_uv_binary_linux, "--version"], 
                                              capture_output=True, text=True)
                        if result.returncode == 0:
                            print(f"[{PkTexts.LINUX_UV_INSTALLATION_COMPLETE}] {PkColors.CYAN}버전={result.stdout.strip()} {PkColors.RESET}")
                            return True
                        else:
                            print(f"[{PkTexts.LINUX_UV_BINARY_ALREADY_INSTALLED}] {PkColors.CYAN}경로={self.f_uv_binary_linux} {PkColors.RESET}")
                            return True
                    else:
    ...
    def install_fzf(self, max_retry: int = 2) -> bool:
        ...
        try:
            ...
            result = subprocess.run([self.f_fzf_binary_linux, "--version"], 
                                  capture_output=True, text=True)
            if result.returncode == 0:
                print(f"[{PkTexts.LINUX_FZF_ALREADY_INSTALLED}] {PkColors.CYAN}버전={result.stdout.strip()} {PkColors.RESET}")
                return True
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        result = subprocess.run([self.f_fzf_binary_linux, "--version"], 
                                              capture_output=True, text=True)
                        if result.returncode == 0:
                            print(f"[{PkTexts.LINUX_FZF_INSTALLATION_COMPLETE}] {PkColors.CYAN}버전={result.stdout.strip()} {PkColors.RESET}")
                            return True
                        else:
                            print(f"[{PkTexts.LINUX_FZF_BINARY_ALREADY_INSTALLED}] {PkColors.CYAN}경로={self.f_fzf_binary_linux} {PkColors.RESET}")
                            return True
                    else:
            ...
            try:
                ...
                if 'pk_system PATH 설정' not in content:
                    ...
                    with open(config_file, 'a') as f:
                        f.write(path_config)
                    print(f"[{PkTexts.LINUX_SHELL_PATH_SETUP_COMPLETE}] {PkColors.CYAN}파일={config_file} {PkColors.RESET}")
                else:
                    print(f"[{PkTexts.LINUX_SHELL_PATH_SETUP_COMPLETE}] {PkColors.CYAN}파일={config_file} {PkColors.RESET}")
                    
            except FileNotFoundError:
                print(f"[{PkTexts.LINUX_SHELL_CONFIG_NOT_FOUND}] {PkColors.YELLOW}파일={config_file} {PkColors.RESET}")
                # 파일 생성
                with open(config_file, 'w') as f:
                    f.write(path_config)
                print(f"[{PkTexts.LINUX_SHELL_PATH_SETUP_COMPLETE}] {PkColors.CYAN}파일생성={config_file} {PkColors.RESET}")
    ...
    def setup_aliases(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                print(f"[{PkTexts.LINUX_ALIAS_SETUP_COMPLETE}] {PkColors.CYAN}상태=완료 {PkColors.RESET}")
                return True
                
            except Exception as e:
                print(f"[{PkTexts.LINUX_ALIAS_SETUP_FAILED}] {PkColors.RED}오류={e} {PkColors.RESET}")
                return False
    ...
    def sync_uv_packages(self) -> bool:
        ...
        try:
            ...
            if pyproject_file.exists():
                print(f"[{PkTexts.LINUX_PYPROJECT_TOML_FOUND}] {PkColors.CYAN}경로={work_root} {PkColors.RESET}")
            else:
                # 상위 디렉토리에서 찾기
                for parent in work_root.parents:
                    ...
                    if pyproject_file.exists():
                        work_root = parent
                        print(f"[{PkTexts.LINUX_PYPROJECT_TOML_OR_ROOT_FOUND}] {PkColors.CYAN}경로={work_root} {PkColors.RESET}")
                        break
                else:
            ...
            if activate_script.exists():
                ...
                if result.returncode == 0:
                    print(f"[{PkTexts.LINUX_UV_SYNC_COMPLETE}] {PkColors.CYAN}상태=완료 {PkColors.RESET}")
                    return True
                else:
    ...
    def install_packages_with_venv_python(self) -> bool:
        ...
        try:
            ...
            if not self.f_venv_python_linux.exists():
                print(f"[{PkTexts.LINUX_VENV_PYTHON_NOT_FOUND}] {PkColors.RED}경로={self.f_venv_python_linux} {PkColors.RESET}")
                return False
            
            print(f"[{PkTexts.LINUX_VENV_PYTHON_FOUND}] {PkColors.CYAN}경로={self.f_venv_python_linux} {PkColors.RESET}")
            ...
            for package in packages:
                try:
                    print(f"[{PkTexts.LINUX_PACKAGE_INSTALLING}] {PkColors.YELLOW}패키지={package} {PkColors.RESET}")
                    ...
                    if result.returncode == 0:
                        print(f"[{PkTexts.LINUX_PACKAGE_INSTALL_COMPLETE}] {PkColors.CYAN}패키지={package} {PkColors.RESET}")
                    else:
                        print(f"[{PkTexts.LINUX_PACKAGE_INSTALL_FAILED}] {PkColors.RED}패키지={package} 오류={result.stderr} {PkColors.RESET}")
                        
                except Exception as e:
                    print(f"[{PkTexts.LINUX_PACKAGE_INSTALL_FAILED}] {PkColors.RED}패키지={package} 오류={e} {PkColors.RESET}")
