...
def ensure_ha_token_obtained(
    ...
    """
    logging.info("Home Assistant Long-Lived Access Token 생성 안내")
    ...
    if open_browser:
        try:
            ...
            logging.info("브라우저에서 토큰 생성 페이지를 열었습니다.")
        except Exception as e:
            logging.warning("브라우저를 열 수 없습니다: %s", e)
            logging.info("수동으로 다음 URL을 열어주세요: %s", token_url)
    ...
    if not ha_token:
        logging.warning("토큰이 입력되지 않았습니다.")
        return None
    return ha_token
        ...
        if not ha_url:
            ...
            try:
                ...
                if ha_ip:
                    ...
                    logging.error("HA URL이 선택되지 않았습니다.")
                    return None
            except Exception as e:
                logging.error("HA URL을 가져오는 중 오류 발생: %s", e)
                return None
