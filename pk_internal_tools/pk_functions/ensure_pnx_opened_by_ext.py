def ensure_pnx_opened_by_ext(pnx, logging_mode=True):
    import logging
    import time
    import os
    ...
    try:
        ...
        # TODO : ensure_value_complete 로 opener 설정 (TBD)
        if is_os_windows():
            ...
            if get_os_n() == 'windows':
                ...
                if F_PYCHARM_EXE:
                    ext_to_program['log'] = (f'{F_PYCHARM_EXE}', 'log file, opening in PyCharm')
                    ext_to_program['py'] = (str(F_PYCHARM_EXE), 'python file, opening in PyCharm')
                    ext_to_program['bat'] = (str(F_PYCHARM_EXE), 'batch file, opening in PyCharm')
