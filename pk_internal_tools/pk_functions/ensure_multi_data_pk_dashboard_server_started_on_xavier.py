"""
Xavier에서 멀티 데이터 대시보드 서버를 시작하는 함수
P110M, TV, Arduino 등 여러 데이터 소스를 조회/제어할 수 있는 통합 대시보드
"""
...
def ensure_multi_data_pk_dashboard_server_started_on_xavier(
    host: str = "0.0.0.0",
    port: int = 8000,
    ...
    """
    Xavier에서 멀티 데이터 대시보드 서버를 시작합니다.
    
    P110M, TV, Arduino 등 여러 데이터 소스를 조회/제어할 수 있는 통합 대시보드를 제공합니다.
    ...
    try:
        ...
        local_script_path = Path(__file__).parent / "ensure_multi_data_pk_dashboard_server_on_xavier.py"
        
        if not local_script_path.exists():
            logger.error("멀티 데이터 대시보드 서버 파일을 찾을 수 없습니다: %s", local_script_path)
            return False
        
        # Xavier에 대시보드 서버 스크립트 전송
        remote_script_path = "/tmp/ensure_multi_data_pk_dashboard_server_on_xavier.py"
        
        try:
            logger.info("멀티 데이터 대시보드 서버 스크립트를 Xavier에 전송 중...")
            ...
            logger.info("Xavier에서 멀티 데이터 대시보드 서버를 시작합니다...")
            ...
            env_vars = f"MULTI_DATA_DASHBOARD_HOST={host} MULTI_DATA_DASHBOARD_PORT={port}"
            cmd = f"nohup env {env_vars} python3 {remote_script_path} > /tmp/multi_data_pk_dashboard_server.log 2>&1 &"
            ...
            if exit_code == 0:
                logger.info("✅ 멀티 데이터 대시보드 서버가 Xavier에서 시작되었습니다.")
                logger.info("📊 접속 URL: http://%s:%d", xavier_ip, port)
                logger.info("서버 로그 확인: ssh로 Xavier 접속 후 'tail -f /tmp/multi_data_pk_dashboard_server.log'")
                logger.info("서버 중지: Xavier에서 'pkill -f ensure_multi_data_pk_dashboard_server_on_xavier.py'")
                return True
            else:
                logger.error("멀티 데이터 대시보드 서버 시작 실패")
                if stderr:
                    for line in stderr:
                        logger.error("  %s", line)
                return False
        ...
        logger.error(f"Xavier 멀티 데이터 대시보드 서버 시작 중 예외 발생: {e}", exc_info=True)
        return False
