...
def ensure_pk_yeelight_on(
    ...
    """
    Orchestrates the full flow to turn a Yeelight device ON:
    provisioning, token retrieval, IP discovery, optional developer mode setup,
    and finally, setting the state to "on".
    ...
    Returns:
        True if the Yeelight is successfully turned ON, False otherwise.
    """
    func_n = get_caller_name()

    logging.info(PK_UNDERLINE)
    logging.info(f"{PkColors.BRIGHT_CYAN}YeeLight ON 제어 흐름 시작{PkColors.RESET}")
    logging.info(PK_UNDERLINE)
    ...
    try:
        ...
        if controller is None:
            logging.error("Xavier 원격 컨트롤러 확보 실패. YeeLight ON 제어를 중단합니다.")
            return False
        ...
        # 5. State Change (ON)
        logging.info(f"YeeLight ({yeelight_ip}) 상태를 'on'으로 변경 시도합니다...")
        ...
        if control_success:
            logging.info(f"✅ YeeLight '{yeelight_ip}' 상태를 'on'으로 성공적으로 변경했습니다.")
            return True
        else:
            logging.error(f"YeeLight '{yeelight_ip}' 상태를 'on'으로 변경하는 데 실패했습니다.")
            return False

    except Exception as e:
        logging.error(f"YeeLight ON 제어 흐름 중 오류 발생: {e}", exc_info=True)
        ensure_debugged_verbose(traceback, e)
        return False
    finally:
        logging.info(PK_UNDERLINE)
        logging.info(f"{PkColors.BRIGHT_CYAN}YeeLight ON 제어 흐름 종료{PkColors.RESET}")
        logging.info(PK_UNDERLINE)
