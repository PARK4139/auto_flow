...
def ensure_instagram_cookies_created(force_refresh=False):
    ...
    """
    cookie_dir = Path(d_pk_root_hidden)
    cookie_file = cookie_dir / "instagram_cookies.txt"
    cookie_meta_file = cookie_dir / "instagram_cookies_metadata.json"
...
def _diagnose_cookie_status(cookie_file: Path, cookie_meta_file: Path) -> str:
    ...
    if cookie_meta_file.exists():
        try:
            ...
            return "invalid"
    return "valid" # 메타데이터 없으면 일단 유효하다고 가정
...
def _guide_user_for_manual_cookie_creation(cookie_file: Path) -> bool:
    ...
    n. 'Cookie-Editor' 아이콘을 마우스 우클릭
    n. 'Export' 버튼 (네 번째 아이콘)을 클릭  
    n. 'Export format'을 'Netscape'로 선택 # json 으로 복사했는데도 되네.  
    n. 쿠키 정보가 클립보드에 복사되었으면, 아래 질문에 '예'라고 답해주세요.
    ------------------------------------------------------------------------------------
    """)
    ...
    user_response = ensure_value_completed_2025_10_12_0000(
        key_name="위 가이드에 따라 쿠키 정보를 클립보드에 복사하셨습니까?",
    ...
    try:
        ...
        edit_response = ensure_value_completed_2025_10_12_0000(
