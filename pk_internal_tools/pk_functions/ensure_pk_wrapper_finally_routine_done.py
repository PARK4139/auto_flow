def ensure_pk_wrapper_finally_routine_done(*, project_root_directory, traced_file, console_log_block_mode=True, terminate_spoken_manager=True):
    import logging
    import traceback
    ...
    try:
        ...
        if QC_MODE:
            ...
            if not any(name in traced_file for name in exceptions):
                # ensure_pk_system_log_editable() # pk_option
                pass
        ...
        if not QC_MODE:
            script_to_run = rf"{project_root_directory}\.venv\Scripts\activate && python {traced_file} && deactivate"
            logging.debug(PK_UNDERLINE)
            logging.debug(f"'{PkTexts.TRY_GUIDE}' {script_to_run}")
            logging.debug(PK_UNDERLINE)

        get_pk_spoken_manager().wait_for_completion()  # 완전히 재생될때 까지 # 모든 큐 작업이 완료될 때까지 flow 더 흘러가지 못하도록, 블로킹
        if terminate_spoken_manager:
