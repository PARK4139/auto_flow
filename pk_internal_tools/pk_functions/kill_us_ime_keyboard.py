def kill_us_ime_keyboard():
    ...
    def kill_us_ime_keyboard_in_loop():
        """flag 값이 True가 되면 특정 작업 수행 후 다시 False로 초기화"""
        try:
            existing_shm = shared_memory.SharedMemory(name=shm_name)
            flag = existing_shm.buf
        except FileNotFoundError:
            logging.debug("kill_us_ime_keyboard_in_loop: 공유 메모리가 존재하지 않음. 종료.")
            return

        while 1:
            # exec 할 명령
            f_cmd = rf"{D_PK_EXTERNAL_TOOLS}/ensure_us_keyboard_killed.cmd"
            f_cmd = Path(f_cmd)
            ensure_command_executed(cmd=rf'"{f_cmd}"')
