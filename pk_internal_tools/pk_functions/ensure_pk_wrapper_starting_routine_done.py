...
def ensure_pk_wrapper_starting_routine_done(*, traced_file, traceback, window_relocate_mode=True, start_window_manager_thread=True):
    import logging
    import sys
    ...
    if "pytest" not in sys.modules:
        ...
        logging.warning("Pytest is running, skipping suicide routine.")
    ensure_pk_colorama_initialized_once()
    ensure_pk_system_log_initialized(file_path=traced_file)
    ...
    # ensure_window_titles_closed_like_person()

    if start_window_manager_thread:
