...
def _collect_pk_interesting_info_field(
    ...
    flags: SetupOpsForGetPkInterestingInfo,
    field_flag: SetupOpsForGetPkInterestingInfo,
    collector_func,
    error_message: str,
    default_value,
    field_name: str
...
def _collect_date_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.DATE,
...
def _collect_time_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.TIME,
...
def _collect_day_of_week_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    from pk_internal_tools.pk_functions.get_weekday import get_weekday
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.DAY_OF_WEEK,
...
def _collect_location_weather_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    if flags & SetupOpsForGetPkInterestingInfo.LOCATION_WEATHER:
        try:
...
def _collect_stock_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    if flags & SetupOpsForGetPkInterestingInfo.STOCK:
        try:
...
def _collect_os_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    from pk_internal_tools.pk_functions.get_os_n import get_os_n
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.OS,
...
def _collect_connected_drives_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    from pk_internal_tools.pk_functions.get_connected_drives_info import get_connected_drives_info
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.CONNECTED_DRIVES,
...
def _collect_screen_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    from pk_internal_tools.pk_functions.get_pk_screen_info import get_pk_screen_info
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.SCREEN,
...
def _collect_project_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    from pk_internal_tools.pk_functions.get_project_info_from_pyproject import get_project_info_from_pyproject
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.PROJECT,
...
def _collect_window_titles_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    from pk_internal_tools.pk_functions.get_window_titles import get_window_titles
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.WINDOW_TITLES,
...
def _collect_tasklist_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    from pk_internal_tools.pk_functions.get_tasklist_with_pid import get_tasklist_with_pid
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.TASKLIST,
...
def _collect_image_names_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    from pk_internal_tools.pk_functions.get_image_names_from_tasklist import get_image_names_from_tasklist
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.IMAGE_NAMES_INFO,
...
def _collect_top_processes_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    if flags & (SetupOpsForGetPkInterestingInfo.TOP_CPU_PROCESSES | SetupOpsForGetPkInterestingInfo.TOP_MEMORY_PROCESSES):
        try:
            ...
            if flags & SetupOpsForGetPkInterestingInfo.TOP_CPU_PROCESSES:
                data.top_cpu_processes = top_processes_result.get('top_cpu', [])
            if flags & SetupOpsForGetPkInterestingInfo.TOP_MEMORY_PROCESSES:
...
def _collect_processes_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    from pk_internal_tools.pk_functions.get_process_infos import get_process_infos
    _collect_pk_interesting_info_field(
        data=data,
        flags=flags,
        field_flag=SetupOpsForGetPkInterestingInfo.PROCESSES,
...
def _collect_wifi_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo, func_n):
    if flags & SetupOpsForGetPkInterestingInfo.WIFI:
        try:
            ...
            wifi_password = ensure_env_var_completed_2025_11_24(key_name="WIFI_PW", func_n=func_n, mask_log=True)
...
def _collect_python_imports_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    if flags & SetupOpsForGetPkInterestingInfo.PYTHON_IMPORTS:
        try:
...
def _collect_ai_ide_processes_info(data: PkInterestingInfos, flags: SetupOpsForGetPkInterestingInfo):
    if flags & SetupOpsForGetPkInterestingInfo.AI_IDE_PROCESSES_INFO:
        try:
