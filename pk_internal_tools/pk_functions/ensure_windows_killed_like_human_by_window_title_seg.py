...
def ensure_windows_killed_like_human_by_window_title_seg(window_title_seg):
    # TODO : ensure_process_killed_by_window_title 보다 나은 느낌은 들었음. 검증필요
    ...
    while True:
        ...
        if not is_window_opened(window_title_seg=window_title_seg):
            ...
            if is_window_title_front(window_title=window_title_seg):
                ...
                logging.debug(f'{window_title_seg} is tried to close')
            else:
                logging.debug(f'{window_title_seg}이 앞에 있지 않아 창 닫기를 시도하지 않았습니다.')
