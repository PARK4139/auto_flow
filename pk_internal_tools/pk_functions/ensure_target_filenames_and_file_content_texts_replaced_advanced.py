...
def ensure_target_filenames_and_file_content_texts_replaced_advanced(d_target, old_text, new_text, target_extensions=None, ignored_directory_names=None, operation_mode=None):
    import os
    import traceback
    from pathlib import Path

    from pk_internal_tools.pk_functions.ensure_target_filenames_and_file_content_texts_replaced import ensure_target_filename_and_file_content_text_replaced
    ...
    if operation_mode is None:
        operation_mode = SetupOpsForEnsureTargetFilenameAndFileContentTextReplaced.FILENAME_AND_FILECONTENT
    ...
    logging.debug(f'd_target={d_target_path} {"%%%FOO%%%" if QC_MODE else ""}')

    if not d_target_path.is_dir():
        logging.error(f" 디렉토리 없음: {d_target_path}")
        return
    ...
    for root, dirs, files in os.walk(str(d_target_path), topdown=False):
        ...
        for filename in files:
            ...
            ensure_target_filename_and_file_content_text_replaced(f_target=str(file_path_old), old_text=old_text, new_text=new_text, operation_mode=operation_mode)
    ...
    for dir_path_old, dir_path_new in dirs_to_rename:
        try:
            ...
            logging.error(f" 디렉토리 이름 변경 실패: {dir_path_old} -> {dir_path_new}")
            logging.debug(e)
            traceback.print_exc()
