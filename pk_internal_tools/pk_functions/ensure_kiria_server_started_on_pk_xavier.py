...
def _check_microphone_on_pk_xavier(controller: PkRemoteTargetEngine) -> bool:
    """Xavier에서 마이크가 사용 가능한지 확인"""
    logging.info("Xavier에서 마이크 확인 중...")

    # 1. USB 마이크 확인
    logging.info("1. USB 마이크 확인 중...")
    ...
    # 2. ALSA 오디오 장치 확인
    logging.info("2. ALSA 오디오 장치 확인 중...")
    stdout, stderr, exit_code = controller.ensure_command_to_remote_target(
    ...
    # 3. Python으로 마이크 확인
    logging.info("3. Python으로 마이크 확인 중...")
    mic_check_script = """
import sys
try:
    import speech_recognition as sr
...
"""
    ...
    try:
        ...
        if ok:
            ...
            if exit_code == 0:
                logging.info("✅ 마이크가 감지되었습니다.")
                return True
            elif exit_code == 2:
                logging.warning("⚠️ SpeechRecognition 라이브러리가 설치되지 않았습니다.")
                return False
            else:
                logging.warning("⚠️ 마이크가 감지되지 않았습니다.")
                return False
        else:
            logging.warning("⚠️ 마이크 확인 스크립트 전송 실패")
            return False
    finally:
        import os
...
def _ensure_microphone_dependencies_on_pk_xavier(controller: PkRemoteTargetEngine) -> bool:
    """Xavier에 마이크 사용을 위한 필수 패키지 설치"""
    logging.info("마이크 사용을 위한 필수 패키지 설치 중...")
    ...
    for package in packages:
        logging.info("%s 설치 중...", package)
        stdout, stderr, exit_code = controller.ensure_command_to_remote_target(cmd=f"sudo apt-get install -y {package}", timeout_seconds=120)
        if exit_code != 0:
            logging.warning("  ⚠️ %s 설치 실패 (계속 진행)", package)

    # 2. Python 라이브러리 설치
    logging.info("Python 음성 인식 라이브러리 설치 중...")
    stdout, stderr, exit_code = controller.ensure_command_to_remote_target(
    ...
    if exit_code != 0:
        logging.warning("  ⚠️ Python 라이브러리 설치 실패")
        if stderr:
            for line in stderr:
                logging.debug("    %s", line)

    # 3. 오디오 그룹에 사용자 추가
    logging.info("오디오 그룹 권한 확인 중...")
    stdout, stderr, exit_code = controller.ensure_command_to_remote_target(
    ...
    if stdout and any("추가 필요" in line for line in stdout):
        logging.info("사용자를 audio 그룹에 추가 중...")
        user = getattr(controller.remote_target, "user_n", "pk")
        stdout, stderr, exit_code = controller.ensure_command_to_remote_target(cmd=f"sudo usermod -a -G audio {user}", timeout_seconds=10)
        if exit_code == 0:
            logging.info("audio 그룹에 추가되었습니다. (재로그인 필요할 수 있음)")
...
try:
    ...
    logging.info("kiri 서버 시작 중...")
    ensure_pk_kiria_executed_on_pk_xavier()
except ImportError as e:
    logging.error("kiri 모듈을 찾을 수 없습니다: %s", e)
    logging.info("프로젝트 경로 확인: %s", pk_root)
    sys.exit(1)
except Exception as e:
    logging.error("kiri 서버 실행 중 오류: %s", e, exc_info=True)
    sys.exit(1)
""")


def ensure_kiria_server_started_on_pk_xavier(
        pk_xavier_ip: Optional[str] = None,
        pk_xavier_user: Optional[str] = None,
        pk_xavier_pw: Optional[str] = None,
    ...
    Args:
        pk_xavier_ip: Xavier IP 주소. None이면 환경변수 또는 입력받기
        pk_xavier_user: Xavier 사용자명. None이면 환경변수 또는 입력받기
        pk_xavier_pw: Xavier 비밀번호. None이면 환경변수 또는 입력받기
    ...
    try:
        ...
        # Xavier 연결 정보 가져오기
        if not pk_xavier_ip:
            pk_xavier_ip = ensure_env_var_completed("XAVIER_IP")
        if not pk_xavier_user:
            pk_xavier_user = ensure_env_var_completed("XAVIER_USER")
        if not pk_xavier_pw:
            pk_xavier_pw = ensure_env_var_completed("XAVIER_PW")

        # Xavier 컨트롤러 생성
        controller = PkRemoteTargetEngine(
            
            ip=pk_xavier_ip,
            user_n=pk_xavier_user,
            pw=pk_xavier_pw,
        )

        logging.info("=" * 60)
        logging.info("kiri 서버 시작 (Xavier)")
        logging.info("=" * 60)
        ...
        if not skip_mic_check:
            mic_available = _check_microphone_on_pk_xavier(controller)

            if not mic_available:
                logging.warning("⚠️ 마이크가 감지되지 않았습니다.")
                logging.info("마이크 설치 가이드를 참고하여 마이크를 연결하고 다시 시도하세요.")

                # 필수 패키지 설치 시도
                logging.info("필수 패키지 설치를 시도합니다...")
                _ensure_microphone_dependencies_on_pk_xavier(controller)

                # 다시 확인
                logging.info("마이크를 다시 확인합니다...")
                mic_available = _check_microphone_on_pk_xavier(controller)

                if not mic_available:
                    logging.warning("⚠️ 마이크가 여전히 감지되지 않습니다.")
                    logging.info(get_text_yellow("마이크 없이도 서버는 시작할 수 있지만, 음성 인식 기능은 사용할 수 없습니다."))
                    response = input("continue:y/n").strip().lower()
                    if response != 'y':
                        logging.info("사용자가 취소했습니다.")
                        return False
        else:
            logging.info("마이크 확인을 건너뜁니다.")
        ...
        # 디렉토리 생성 명령 추가
        logging.info("원격 서버에 pk_service 디렉토리 생성 중...")
        ...
        try:
            logging.info("kiri 서버 스크립트를 Xavier에 전송 중...")
            ...
            logging.info("Xavier에서 kiri 서버를 시작합니다...")
            logging.info("서버를 중지하려면 Xavier에서 Ctrl+C를 누르거나 프로세스를 종료하세요.")
            ...
            if exit_code == 0:
                ...
                if mic_available:
                    logging.info("✅ 마이크가 감지되어 음성 인식 기능을 사용할 수 있습니다.")
                else:
                    logging.info("⚠️ 마이크가 감지되지 않아 키보드 입력 모드로 동작합니다.")
                logging.info("서버 로그 확인: ssh로 Xavier 접속 후 'tail -f /tmp/pk_service/pk_kiria_server.log'")
                logging.info("서버 중지: Xavier에서 'pkill -f pk_kiria_server.py'")
                return True
            else:
        ...
        logging.error(f"Xavier kiri 서버 시작 중 예외 발생: {e}", exc_info=True)
        return False
