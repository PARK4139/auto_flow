def get_data_required_from_f_csv(line_order):
    ...
    from pk_internal_tools.pk_functions.get_df_from_issues_csv import get_df_from_issues_csv
    from pk_internal_tools.pk_functions.get_nth_row import get_nth_row
    from pk_internal_tools.pk_objects.pk_texts import PK_UNDERLINE

    from pk_internal_tools.pk_functions.get_caller_name import get_caller_name
    func_n = get_caller_name()
    logging.debug(rf'''{func_n}()  ''')
    df = get_df_from_issues_csv()
    ...
    if nth_row is not None:
        logging.debug(rf'''{PK_UNDERLINE}n="{line_order}"  ''')
