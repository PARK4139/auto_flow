...
class PerformanceBenchmark:
    ...
    def __init__(self, output_dir: Optional[Path] = None):
        ...
        self.output_dir = output_dir or D_PK_LOGS
    ...
    def _save_results(self):
        ...
        file_name = f"performance_benchmark_{timestamp}.json"
        filepath = self.output_dir / file_name
    ...
    def export_report(self, file_name: Optional[str] = None) -> Path:
        """상세한 벤치마크 리포트를 생성합니다."""
        if not file_name:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            file_name = f"performance_report_{timestamp}.md"
...
def export_benchmark_report(file_name: Optional[str] = None) -> Path:
    """벤치마크 리포트를 파일로 내보냅니다."""
    return _benchmark.export_report(file_name)
