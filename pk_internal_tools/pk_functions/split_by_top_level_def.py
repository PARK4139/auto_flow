def split_by_top_level_def(d_working, filepath, prefix=None, preview=False):
    import logging
    import logging
    from pk_internal_tools.pk_functions.get_unique_file_name import get_unique_file_name
    import os.path
    import re
    ...
    for line in lines:
        ...
        if def_match and len(def_match.group(1)) == 0:
            ...
            current_func_name = get_sanitized_file_path(def_match.group(2))
    ...
    if preview:
        ...
        for i, name in enumerate(chunk_names):
            ...
            file_name = f"{base}{suffix}.py"

            color_start = "\033[91m" if duplicated_name_counter[name] > 1 else ""
            color_end = "\033[0m" if duplicated_name_counter[name] > 1 else ""

            logging.debug(f"{color_start}[{PkTexts.SPLIT_PLAN}] def {name}() → [{PkTexts.WILL_SAVE_AS}] {file_name}{color_end}")
    ...
    for i, (chunk, name) in enumerate(zip(chunks, chunk_names)):
        ...
        file_name = f"{base}{suffix}.py"
        file_name = get_unique_file_name(file_name.replace('.py', ''), d_working)
        output_path = os.path.join(d_working, file_name)
        ...
        logging.debug(f"[{PkTexts.WRITE_DONE}] {file_name}")
