...
def get_new_pnx_deduplicatd_with_suffix_number(base_pnx, target_directory, max_attempts=1000):
    ...
    # 기본 파일명으로 시작
    final_file_name = base_pnx
    target_path = os.path.join(target_directory, final_file_name)
    ...
    while counter <= max_attempts:
        final_file_name = f"{name}_{counter}{ext}"
        target_path = os.path.join(target_directory, final_file_name)
        
        if not os.path.exists(target_path):
            logging.debug(f"중복 해결된 파일명 발견: {final_file_name}")
            return final_file_name
        
        logging.debug(f"파일명 충돌 해결 시도 {counter}: {final_file_name} (이미 존재함)")
        counter += 1
...
def get_new_pnx_deduplicatd_with_suffix_number_v2(base_pnx, target_directory, max_attempts=1000):
    ...
    if not os.path.exists(target_path):
        logging.debug(f"기본 파일명 사용 가능: {base_pnx}")
        return final_file_name, target_path
