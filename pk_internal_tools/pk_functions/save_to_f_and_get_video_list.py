...
def save_to_f_and_get_video_list(f, d_working, allowed_extensions, filename_parts_to_ignore):
    logging.debug(f'''f={f} ''')
    if not is_pnx_existing(pnx=f):
        ensure_pnx_made(pnx=f, mode='f')
    v_f_list = get_files_filtered_from_db(d_working, allowed_extensions, filename_parts_to_ignore)
