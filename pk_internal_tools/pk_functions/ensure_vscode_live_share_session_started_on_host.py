...
def ensure_vscode_live_share_session_started_on_host(
    remote_target_host: Optional[str] = None,
    ...
    Args:
        remote_target_host: Xavier SSH 호스트명 (예: "remote_target"). None이면 환경변수 또는 입력받기
    ...
    try:
        ...
        if not _check_vscode_running():
            ...
            logger.info("    code")
            logger.info("  또는")
            logger.info("    code --remote ssh-remote+remote_target <프로젝트_경로>")
        else:
        ...
        if remote_target_host:
            logger.info("   - 호스트: %s", remote_target_host)
            logger.info("   - 명령: code --remote ssh-remote+%s <프로젝트_경로>", remote_target_host)
        else:
            logger.info("   - F1 > 'Remote-SSH: Connect to Host...'")
            logger.info("   - 또는: code --remote ssh-remote+remote_target <프로젝트_경로>")
        ...
        url_input = ensure_env_var_completed(
