...
# pk_files에서 F_MEDIA_FILES_DB 경로를 가져옵니다.
...
def get_pk_media_files_db_schema():
    """
    media_files.sqlite 데이터베이스의 스키마를 조회하여 반환합니다.
    """
...
if __name__ == '__main__':
    ...
    ensure_pk_wrapper_starting_routine_done(traced_file=__file__, traceback=traceback)
    schema = get_pk_media_files_db_schema()
    if schema:
        logging.info("media_files.sqlite 스키마 정보:")
        for table, columns in schema.items():
            logging.info(f" 테이블: {table}")
            for col in columns:
                logging.info(f"{col['name']} ({col['type']}) {'[PK]' if col['pk'] else ''}")
    else:
        logging.info("media_files.sqlite 스키마 정보를 가져올 수 없습니다.")