...
def ensure_wired_target_controlled():
    """
    ...
    try:
        ...
        from pk_internal_tools.pk_objects.pk_remote_target_controller import PkModes2
        ...
        logging.info(f"{PkColors.BRIGHT_CYAN}유선 타겟 컨트롤러 실행 시작{PkColors.RESET}")
        logging.info(PK_UNDERLINE)
        ...
        if QC_MODE:
            ...
            # wired_task = ensure_value_completed(
            #     key_name="wired_task",
            #     func_n=func_n,
            #     options=wired_tasks,
        ...
        if wired_task == TASK_FLASH_TARGET:
            ...
            tm = PkWiredTargetController(identifier=PkDevice.jetson_agx_xavier)
            ...
            try:
                from pk_internal_tools.pk_functions.ensure_remote_target_wifi_listed import (
                    ensure_remote_target_wifi_listed,
                    ensure_remote_target_wifi_status,
                )

                # 먼저 현재 Wi-Fi 연결 상태 확인
                logging.info("현재 Wi-Fi 연결 상태를 확인합니다...")
                status = ensure_remote_target_wifi_status()
                ...
                wifi_networks = ensure_remote_target_wifi_listed()
            ...
            try:
                from pk_internal_tools.pk_functions.ensure_remote_target_wifi_connected import (
                    ensure_remote_target_wifi_connected,
                )
                ...
                if not wifi_ssid:
                    ...
                    wifi_ssid = ensure_env_var_completed(
                ...
                # Wi-Fi 연결
                success = ensure_remote_target_wifi_connected(wifi_ssid=wifi_ssid)
            ...
            try:
                from pk_internal_tools.pk_functions.ensure_arduino_dev_via_wired_remote_target_wireless_arduino import (
                    ensure_arduino_dev_via_wired_remote_target_wireless_arduino,
                )
                ...
                remote_target_ip = getattr(wired_controller._target, "ip", None) or getattr(wired_controller._target, "hostname", None)
                remote_target_user = getattr(wired_controller._target, "user_n", None) or "pk"
                remote_target_pw = getattr(wired_controller._target, "pw", None)
                ...
                if QC_MODE:
                    ...
                    setup_wifi = ensure_value_completed(
                    ...
                    if setup_wifi == "yes":
                        arduino_wifi_ssid = ensure_value_completed(
                        ...
                        if arduino_wifi_ssid:
                            arduino_wifi_password = ensure_value_completed(
                ...
                # Arduino 개발 환경 설정
                success = ensure_arduino_dev_via_wired_remote_target_wireless_arduino(
            ...
            try:
                from pk_internal_tools.pk_functions.ensure_arduino_dev_via_wired_remote_target_wired_arduino_nano import (
                    ensure_arduino_dev_via_wired_remote_target_wired_arduino_nano,
                )
                ...
                success = ensure_arduino_dev_via_wired_remote_target_wired_arduino_nano(
                    remote_target_ip=remote_target_ip,
                    remote_target_user=remote_target_user,
                    remote_target_pw=remote_target_pw,
                )
            ...
            try:
                ...
                if QC_MODE:
                    ...
                    skip_response = ensure_value_completed(
            ...
            try:
                from pk_internal_tools.pk_functions.ensure_vscode_live_share_server_started_on_remote_target import (
                    ensure_vscode_live_share_server_started_on_remote_target,
                )
                ...
                success = ensure_vscode_live_share_server_started_on_remote_target(
                    remote_target_ip=remote_target_ip,
                    remote_target_user=remote_target_user,
                    remote_target_pw=remote_target_pw,
                )
            ...
            try:
                ...
                remote_target_host = getattr(wired_controller._target, "hostname", None) or "remote_target"

                # 프로젝트 경로 입력받기 (선택사항)
                project_path = None
                if not QC_MODE:
                    project_path = ensure_value_completed(
                        key_name="remote_target_project_path",
                        func_n=func_n,
                ...
                if live_share_url:
                    ...
                    logging.info("Live Share 세션 시작 가이드를 제공했습니다.")
        ...
        logging.info(PK_UNDERLINE)
        logging.info(f"{PkColors.BRIGHT_CYAN}유선 타겟 컨트롤러 실행 종료{PkColors.RESET}")
        logging.info(PK_UNDERLINE)
        return True
    except Exception as e:
