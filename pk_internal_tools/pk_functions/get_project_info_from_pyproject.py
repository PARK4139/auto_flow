    ...
    def __repr__(self):
        try:
            if self.data and isinstance(self.data, dict):
                ...
                return (
                    f"Failed to load {PkColors.RED}pyproject.toml"
                    f"{PkColors.RESET} or cache issue occurred.\n"
                    f"{PkColors.BRIGHT_MAGENTA}Debug Info: "
                    f"project_info = {self.data}, type = {type(self.data)}"
                    f"{PkColors.RESET}"
                )
        except Exception as e:
            return f"{PkColors.RED}[ProjectInfo __repr__ Error]{PkColors.RESET} {e}"
