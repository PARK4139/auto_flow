...
def ensure_target_pnx_names_replaced() -> None:
    ...
    - 모든 사용자 입력은 ensure_value_completed 사용
    ...
    # A) d_working 선택
    selected = ensure_value_completed(key_name="d_working", func_n=func_n, options=[])
    ...
    walking_choice = ensure_value_completed(
    ...
    operation_mode = ensure_value_completed(
    ...
    # F) 최종 적용 여부
    final_choice = ensure_value_completed(key_name="apply_changes", func_n=func_n, options=["yes", "no"])
...
def _get_rules_from_toml(func_n: str, rules_path: Path) -> tuple[list, list, list]:
    ...
    open_choice = ensure_value_completed(
    ...
    proceed_choice = ensure_value_completed(
...
def _get_rules_from_typing(func_n: str) -> tuple[list, list, list]:
    ...
    while True:
        old_val = ensure_value_completed(
        ...
        new_val = ensure_value_completed(
