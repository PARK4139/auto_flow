...
def _check_microphone_on_xavier(controller: PkWirelessTargetController) -> bool:
...
"""
    ...
    try:
        ...
        ok = controller.ensure_file_transferred_to_target(temp_script, remote_script)
        if ok:
            stdout, stderr, exit_code = controller.ensure_command_to_wireless_target(
...
def _ensure_microphone_dependencies_on_xavier(controller: PkWirelessTargetController) -> bool:
    ...
    if stdout and any("추가 필요" in line for line in stdout):
        ...
        user = getattr(controller.wireless_target, "user_n", "pk")
        stdout, stderr, exit_code = controller.ensure_command_to_wireless_target(
...
def _get_kiri_server_script_content() -> str:
...
# kiri 실행
try:
    from pk_internal_tools.pk_functions.ensure_kiria_enalbled import ensure_kiria_executed_2025_10_21
    logger.info("kiri 서버 시작 중...")
    ensure_kiria_executed_2025_10_21()
...
def ensure_kiri_server_started_on_xavier(
    ...
    try:
        ...
        if not skip_mic_check:
            mic_available = _check_microphone_on_xavier(controller)
            ...
            if not mic_available:
                ...
                _ensure_microphone_dependencies_on_xavier(controller)
        ...
        script_content = _get_kiri_server_script_content()
        remote_script_path = "/tmp/ensure_kiri_server_on_xavier.py"
        ...
        try:
            ...
            cmd = f"nohup python3 {remote_script_path} > /tmp/kiri_server.log 2>&1 &"
            stdout, stderr, exit_code = controller.ensure_command_to_wireless_target(
                cmd=cmd,
                timeout_seconds=10,
                use_sudo=False,
            ...
            if exit_code == 0:
                ...
                logger.info("서버 로그 확인: ssh로 Xavier 접속 후 'tail -f /tmp/kiri_server.log'")
                logger.info("서버 중지: Xavier에서 'pkill -f ensure_kiri_server_on_xavier.py'")
                return True
            else:
