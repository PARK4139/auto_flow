...
def ensure_youtube_videos_downloaded_v_2025_10_08_1257():
    import logging
    import os
    import textwrap
    from pathlib import Path
    ...
    try:
        ...
        d_youtube_downloads_cache = Path(d_pk_cache) / "youtube_downloads"
        ...
        # 다운로드 후 동작 선택
        play_option = ensure_value_completed_2025_10_12_0000(key_name="다운로드 후 동영상을 재생할까요?", options=[PkTexts.SKIP, PkTexts.play])
        ...
        logging.error(f"예상치 못한 오류 발생: {e}")  # 예외 객체 e를 메시지에 포함
        ensure_debug_loged_verbose(e)  # 예외 객체 e를 인자로 전달
        return False
            ...
            if "Requested format is not available" in error_msg:
                ...
                try:
                    ...
                    for f in format_list:
                        ...
                        filesize_str = "N/A"
                        if filesize:
                            if filesize > 1024*1024*1024: filesize_str = f"{filesize / (1024*1024*1024):.1f}GB"
                            elif filesize > 1024*1024: filesize_str = f"{filesize / (1024*1024):.1f}MB"
                            elif filesize > 1024: filesize_str = f"{filesize / 1024:.0f}KB"
                            else: filesize_str = f"{filesize}B"
                    ...
                    selected_formats_str = ensure_values_completed_2025_10_23(
...
def _handle_failed_urls_at_startup(pk_db: PkSqlite3) -> bool:
    ...
    for url, msg in failed_items:
        ...
        # fzf에 표시될 색상 적용된 문자열
        colored_display_string = f"{PK_ANSI_COLOR_MAP['BRIGHT_RED']}{url} (오류: {clean_msg}){PK_ANSI_COLOR_MAP['RESET']}"
    ...
    selected_display_items = ensure_values_completed_2025_10_23(
        key_name="재시도 또는 삭제할 실패 URL을 선택하세요 (Tab으로 다중선택)",
        options=display_options,
        multi_select=True  # Explicitly set to True for multi-select context
    )
    if not selected_display_items:
        logging.info("선택된 실패 URL이 없습니다. 계속 진행합니다.")
        return False

    selected_failed_urls = [failed_urls_map[item] for item in selected_display_items]

    action = ensure_value_completed_2025_10_12_0000(
...
def _handle_db_download_mode(d_pnx, ydl_opts, pk_db, play_option, cookie_exists) -> List[Path]:
    ...
    if not pending_urls:
        logging.debug("다운로드할 PENDING 상태의 URL이 DB에 없습니다.")
        return []

    logging.debug(f"DB에서 {len(pending_urls)}개의 PENDING URL을 찾았습니다. fzf로 선택해주세요.")

    # fzf로 다중 선택
    selected_urls = ensure_values_completed_2025_10_23(
...
def _get_urls_from_txt_file(file_path: Path) -> list[str]:
    ...
    while True:
        ...
        if QC_MODE:
            ...
            user_choice = ensure_value_completed_2025_10_12_0000(
...
def _add_urls_to_db(pk_db: PkSqlite3, urls: list[str], ydl_opts: dict, d_pnx: str):
    """
    .txt 파일에서 읽은 URL 목록을 분석하여 유효한 단일 동영상만 DB에 추가하고 중복을 처리합니다.
    ...
    import yt_dlp
    from pk_internal_tools.pk_functions.normalize_youtube_url import normalize_youtube_url
    from pk_internal_tools.pk_functions.ensure_value_completed_2025_10_12_0000 import ensure_value_completed_2025_10_12_0000 # For playlist handling
    import os # For playlist handling
    ...
    with yt_dlp.YoutubeDL(validation_opts) as ydl:
        for raw_url in urls:
            ...
            if info and 'entries' in info:  # Check if info is not None before accessing 'entries'
                ...
                user_choice = ensure_value_completed_2025_10_12_0000(key_name=question, options=["모두 다운로드", "건너뛰기"])
