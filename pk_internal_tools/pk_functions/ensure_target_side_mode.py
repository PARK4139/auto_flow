...
def ensure_target_side_mode (target_device_data, remote_device_target_config):
    ...
    try:
        ...
        # download f (TODO_xavier_launcher)
        os.chdir(d_temp)
        std_list = ensure_command_executed(
            cmd="git clone -b packing_ai_framework --single-branch http://211.171.108.170:8003/ai_dept/TODO_xavier_launcher.git")

        # send f (TODO_xavier_launcher)
        upload_pnx_to_remote_os(local_pnx_src=os.path.join(d_temp, f'TODO_xavier_launcher'),
                                remote_pnx_dst='/home/nvidia/works', **remote_device_target_config)

        ensure_pnx_removed(d_temp)

        # unzip f (TODO_xavier_launcher)
        cmd = f"unzip -o ~/works/TODO_xavier_launcher.zip -d ~/works/TODO_xavier_launcher"
        std_outs, std_err_list = ensure_command_to_remote_os(cmd=cmd, **remote_device_target_config)
        if std_outs == [] or std_err_list == []:
            logging.debug(f'''''')
        else:
        ...
        # edit xc_field.sh
        # todo : chore : No password / ai_framework model packing ver / TODO_xavier_launcher / IP Setting
        # 이것도 자동화 하자
        ...
        # TODO_xavier_launcher.zip rm
        cmd = f"rm -rf ~/works/TODO_xavier_launcher.zip"
        std_outs, std_err_list = ensure_command_to_remote_os_with_pw_via_paramiko(cmd=cmd, **remote_device_target_config)
        if std_outs == [] or std_err_list == []:
            logging.debug(f'''''')
        else:
