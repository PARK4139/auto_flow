...
def _remove_empty_dirs(path_to_scan: Path):
    ...
    for dirpath, dirnames, file_names in os.walk(path_to_scan, topdown=False):
        if not dirnames and not file_names:
            try:
                os.rmdir(dirpath)
...
def ensure_files_organized_by_hash(source_directory: Path, destination_directory: Path, hash_algo: str = 'sha1', depth: int = 2, chunk_size: int = 2):
    ...
    for file_path in files_to_move:
        try:
            ...
            # Use file_name bytes to ensure consistent hashing
