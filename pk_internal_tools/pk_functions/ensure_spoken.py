...
# 이를 통해 ensure_spoken 함수는 비동기적으로 즉시 반환되면서도, 음성이 중간에 끊기 는 문제를 방지할 수 있습니다.
import atexit
import logging  # Moved logging import to top

logging.debug("ensure_spoken.py 모듈 로드 시작.")

from pk_internal_tools.pk_objects.pk_spoken_manager import SpokenManager
...
def ensure_spoken(text="", voice_config=None, verbose=False, read_finished_wait_mode: bool = False):
    ...
    # [GEMINI] The comma splitting logic was too aggressive and caused issues with simple phrases.
    # It has been removed to ensure the entire text is spoken at once, making the function more predictable.
    manager.speak(text)

    if read_finished_wait_mode:
    ...
    # 이 줄을 제거하면 ensure_spoken 함수는 음성 출력을 기다리지 않고 즉시 반환됩니 다.
    # manager.wait_for_completion()