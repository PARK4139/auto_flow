...
try:
    ...
    # P110M 에너지 히스토리 조회 함수 (기존 함수 재사용)
    ...
    # async def control_tv(action: str, entity_id: Optional[str] = None):
    #     """TV 제어 (on, off, toggle)"""
    #     ...
    
    # TODO: Arduino 데이터 조회 API 추가
    # @app.get("/api/arduino/data")
    # async def get_arduino_data(device_id: Optional[str] = None):
    #     """Arduino 데이터 조회"""
    #     ...
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .data-section h2 { color: #667eea; margin-bottom: 16px; }
        .chart-container {
            background: white;
            padding: 20px;
            border-radius: 16px;
...
<body>
    <div class="container">
        <div class="header">
            ...
            <button class="control-btn" onclick="refreshAll()">🔄 전체 새로고침</button>
        </div>
        
        <div class="data-section">
            <div style="margin-bottom: 12px;">
                <button class="tab-btn active" onclick="showTab('p110m')">⚡ P110M</button>
                <button class="tab-btn" onclick="showTab('tv')">📺 TV</button>
                <button class="tab-btn" onclick="showTab('arduino')">🔌 Arduino</button>
            </div>
            
            <div id="p110m-tab" class="tab-content active">
            ...
            <div id="tv-tab" class="tab-content">
                <div class="control-panel">
                    <h2>📺 TV 제어</h2>
                    <p style="color: #666; padding: 20px;">TV 제어 기능은 곧 추가될 예정입니다.</p>
                </div>
            </div>
            
            <div id="arduino-tab" class="tab-content">
                <div class="control-panel">
                    <h2>🔌 Arduino 데이터</h2>
                    <p style="color: #666; padding: 20px;">Arduino 데이터 조회 기능은 곧 추가될 예정입니다.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function refreshAll() {
            if (document.getElementById('p110m-tab').classList.contains('active')) {
                loadEnergyData();
            }
            // TODO: 다른 탭의 데이터도 새로고침
        }
