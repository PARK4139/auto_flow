...
def ensure_window_resized_and_positioned_left_half():
    """
    Resizes the active window to occupy the left half of the primary monitor.

    This function is designed to be deterministic, avoiding the stateful behavior
    of the 'Win + Left' keyboard shortcut. It uses pygetwindow and screeninfo
    ...
    try:
        ...
        # 4. Calculate the target geometry for the left half
        target_width = primary_monitor.width // 2
        target_height = primary_monitor.height
        target_x = primary_monitor.x
        ...
        logging.info(f"Window '{window.title}' was resized and moved to the left half of the screen.")
